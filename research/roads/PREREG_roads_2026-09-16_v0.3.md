# Roads direction, pre-registration, v0.3
...
prereg:
  ...
  date: 2026-09-16
  author: A3
  supersedes: research/roads/PREREG_roads_2026-09-16_v0.2.md   # v0.2, kept unedited
  supersedes_frozen: research/roads/PREREGISTRATION.md          # v0.1b, kept unedited
  answers_signoff: research/eval/signoffs/roads_v0.2.md
  answers_conditions: [C1, C2, C3, C4, C5, C6, C7, C8, C9, C10]
  items_present: [P1, P2, P3, P4, P5, P6, P7, P8, P9, P10, P11, P12, P13, P14, P15, P16]
  result_artifact: "research/roads/results/roads_prereg_v0_3_results.json"
  ...
  side_test_reference: student_t           # C2. NOT normal; see 5.1.2
  side_test_df_rule: "n_det - 3"           # three plane parameters in two dimensions
  side_alpha_per_tier: 0.01                # one-sided wrong-side rate, per tier
  side_alpha_union_bound: 0.02             # Bonferroni over the two tiers, section 5.1.2
  side_alpha_grid: [0.005, 0.01, 0.025]    # stated in alpha, never in z
  side_min_epochs_primary: 3               # two-epoch segments are a declared stratum
  attenuation_geometry: "R-set, section 5.1.6"   # C4. kappa_v, instruments 2 and 2b, gate R0
  existence_floor_rule: "W_floor = max(3 * GSD, 1.0 m)"   # C5(a), declared before measurement
  outcome_seen: false
  data_seen: false
...
**Status: v0.3, written 2026-09-16 by A3, answering the ten conditions C1 to C10
of `research/eval/signoffs/roads_v0.2.md`, which signed v0.2 with conditions.
Nothing is fitted. No dataset has been downloaded by A3. No label has been
computed. No number below is a measurement.**

**v0.2 is superseded and is kept unedited at
`research/roads/PREREG_roads_2026-09-16_v0.2.md`.** It is the record of what A6
signed with conditions, and the sign-off record at
`research/eval/signoffs/roads_v0.2.md` names it by path and by sha256, so editing
it would break the signature it records. It is **not** added to the claim
checker's record class, because it passes both live detectors unedited and a
record-class entry would remove a detector from a file that does not need the
exemption. See section 12.4.6 and condition C7.
...
The three state columns read v0.1b, then v0.2, then this version. v0.2's column is
A6's grading in `research/eval/signoffs/roads_v0.2.md` section 2, not A3's.

| item | section | state in v0.1b | state at v0.2, per A6 | state here |
|---|---|---|---|---|
| P1 hypothesis in falsifiable form | 1 | pass | pass | unchanged |
| P2 unit of analysis | 3.2, 10.6, 11.1, 20 | conditional pass | pass, C9 open | unchanged; the counts still cannot exist |
| P3 label rule | 4, 5, 5.7, 5.8 | **fail** | pass on the text, C9 open on the test | step 5 now carries the t cutoff and `SIDE_2EP` (5.1.2c, 5.7); the tests still cannot run |
| P4 covariates and sources | 6.1, 6.2 | conditional pass | pass | `n_epochs` added for the two-epoch secondary (6.2) |
| P5 model form | 7, 10.7 | pass | pass | unchanged |
| P6 held-out sets named in advance | 10.1, 18 | **fail** | conditional pass, C8 open | unchanged; the segment-level fingerprint arrives at v0.3a |
| P7 primary metric | 10.2, 10.3 | conditional pass | pass | unchanged |
| P8 simple baseline | 9, 10.4 | pass | pass | unchanged |
| P9 failing condition | 1.5 | pass | pass | unchanged |
| P10 stopping rule and multiplicity | 19 | **fail** | pass | the ceiling moves from 48 to 49 for the two-epoch secondary, named as a move (19.1) |
| P11 leakage self-audit | 20 | **fail** | present, comparison lost | C10's prohibition written as a phrasing rule with its spellings (20.5) |
| P12 data provenance | 21 | **fail** | present, cannot reach verified | unchanged. C1, and not A3's (21.1) |
| P13 compute and feasibility | 22.4 | **fail** | pass | one more fit in the ceiling; instruments 2 and 2b still never dropped (22.4) |
| P14 scope declaration | 14 | pass | pass | unchanged |
| P15 what the result will not say | 12.4.4, 1.4, 6.7, 7.7.1 | pass | pass, upgraded | RC-011 has landed; the `docs/` reach is C7 and is WJ-002 (12.4.6) |
| P16 shape of the result artifact | 22 | **fail** | pass | key paths added for C2, C3, C4, C5 and C6, and the artifact renamed to v0_3 (22) |

### 0.2 The seven reasons of the v0.1b refusal, answered by number

Kept as the record of how v0.2 reached its shape. The live item list for this version
is the condition table in section 0.4.
...
### 0.3 Where A3 disagreed with the refusal of v0.1b, and how A6 ruled

All three disagreements recorded in v0.2 section 17.3 were **upheld** by A6 in
`research/eval/signoffs/roads_v0.2.md` section 5. They are kept in section 17.3
with A6's ruling attached to each, because a disagreement that was conceded is
part of the record of how this design reached its present shape, not a live
dispute. Section 17.5 is new and holds the two places where A3 reads A6's **v0.2
conditions** differently. Neither changes what this version does: the condition is
satisfied as written in both cases and the reading is recorded beside it.

### 0.4 The ten conditions, and where each one is answered

`research/eval/signoffs/roads_v0.2.md` section 9 carries ten conditions. Eight are
blocking for the primary result and two for a named secondary claim. Three of the
ten are not A3's to satisfy. This table is the index; the state column says what
this document can and cannot do about each.

| id | what it requires, in one line | owner | where v0.3 answers it | state after v0.3 |
|---|---|---|---|---|
| **C1** | every dataset supplying a barrier, covariate, scene, detection or weather value reaches `verified` with a sha256 before any fit on real labels | A1, John | 21.1, and gate C1 added to 11.3 as a hard precondition | **open, not A3's.** WJ-017, WJ-009 and WJ-001 are unchanged since v0.2. Nothing in this session moves it |
| **C2** | the side cutoff restated as a Student's t quantile at `n_det - 3` at a declared one-sided alpha; angle table recomputed; two-epoch segments handled; the two tiers reported at their own operating characteristic | A3 | 5.1.2, 5.1.3, 5.1.5, 12.8, 22.2 | **closed in the design.** The cutoff is `t_{1 - alpha}(n_det - 3)` at `alpha` = 0.01 per tier, union bound 0.02. Verification needs A6 to re-derive the table, which it can do from this text |
| **C3** | the `SIDE` and `TIMING` exclusions checked for outcome dependence by the side-invariant unordered pair of the two bands' burned fractions, both readings written in advance | A3 | 12.9.1, 22.2 | **closed in the design.** The statistic, both readings and the width-tercile stratification are written before any value exists |
| **C4** | the attenuation and recoverability instruments pinned to the geometry that will be fitted; `kappa_v` reported on both sets; gate R0 reads the retained one | A3 | 5.1.6 (the R-set), 11.5.1, 11.5.2, 11.5.2b, 11.3, 12.7, 22.2 | **closed in the design**, with one residual A3 adds rather than hides: the R-set is still a superset of the labelled set, so a third value is reported after labelling and the overstatement is measured (5.1.6) |
| **C5** | `NOEXIST` treated as the width measurement's null result: a declared floor before measurement, `NOEXIST` and `WIDTH` in the 12.9 readings, E1's double duty in its consequence text | A3 | 3.4, 11.3 gate E1, 12.9, 22.2 | **closed in the design.** The floor is a rule evaluated at the delivered ground sample distance, with the number it takes at each plausible delivery, and a declared tripwire at 2 m |
| **C6** | step A of the attribution test records the time margin it implicitly asserts, and the thin-margin count is reported per fire | A3 | 5.6, 22.2 | **closed in the design** |
| **C7** | the vocabulary rule reaches the surfaces a number is quoted on; until WJ-002 lands, no width number leaves `research/`. Plus the record-class narrowing of the sign-off's section 7 | orchestrator, John | 12.4.6, 13, 22.2 | **partly closed.** The record-class narrowing is already landed by the orchestrator in `research/shared/check_research_claims.py` and is recorded in 12.4.6. The `docs/` reach is WJ-002 and is not A3's. The standing constraint is written here |
| **C8** | the segment-level split fingerprint pinned at v0.3a, computed over the committed layout row order before any label | A3 | 16, 18.2 | **deferred by construction.** The layout has not run. The row order is pinned here so the value cannot be chosen after the fact |
| **C9** | both label-rule passes run before any width number is quoted | A3, A6, orchestrator | 5.8, 13 | **deferred by construction.** No candidate set exists, so neither pass can run |
| **C10** | section 20 is not summarised as agreement between two audits, and the P11 order is reversed for the other two directions | orchestrator, A4, A5 | 20.1, 20.5 | **closed for the part A3 owns.** The prohibition is written into 20.5 as a forbidden phrasing with its spellings. The order reversal for landslides and suppression is not A3's to impose |

Three further findings in the sign-off record are not numbered conditions but are
answered here because A6 asked for them by name: the grazing truncation reaching
`kappa_v` (5.1.4), which geometry instruments 2 and 2b run on (5.1.6, part of C4),
and A6's revised expectation that gate W1 fails before gate M1 (17.2, and check W0
in 11.3, which makes the ordering a measured quantity instead of a prediction).
...
4. **The existence check comes free with the width measurement, and that is the
   problem as well as the mitigation.** Width is measured for every segment from
   **pre-fire** orthoimagery (section 6.3.2), so the same imagery answers whether
   the road was there: a segment whose automatic extraction finds no canopy gap at
   the mapped centreline in pre-fire imagery is a road that did not exist, or was
   not visible, before the fire. It exits with reason `NOEXIST` and is counted.
   This makes the orthoimagery gate WJ-009 load-bearing twice, which is worth
   saying plainly, because it means WJ-009 decides more than the width covariate.

#### 3.4.1 `NOEXIST` is the width measurement's null result, not an existence fact

**This is condition C5 and A6's statement of it is sharper than v0.2's own.** A
segment exits `NOEXIST` exactly when the orthoimagery classifier finds no canopy gap
at the mapped centreline. That is the same instrument, reading the same image, that
...
The exclusion therefore runs against **narrow** roads, which section 12.9 names as
the dangerous direction, the one that inflates an apparent width slope rather than
attenuating it.

Three things are pre-registered, corresponding to C5(a), C5(b) and C5(c).

**(a) The minimum detectable gap width is declared now, as a rule with its values,
before any measurement exists.** The imagery has not been obtained, so its ground
sample distance is unknown, and a bare number declared against an unknown resolution
would be a number chosen later in disguise. What is declared instead is the rule and
the value it takes at every plausible delivery, which is a commitment of the same
strength because the rule is fixed and the delivery is not A3's to choose.

> **`W_floor = max( 3 * GSD, 1.0 m )`**, where `GSD` is the per-segment ground sample
> distance of the pre-fire orthoimage actually used. `NOEXIST` means **"no canopy gap
> at or above `W_floor` was found"**, and it is reported under that name and never as
> "the road did not exist". The factor of three is the standard requirement that a
> linear feature span enough pixels to be delineated rather than aliased; the 1.0 m
> absolute floor is there because a sub-metre canopy gap is not a road under any
> Korean forest road class. The per-segment `GSD` and the imagery vintage are
> reported beside every `NOEXIST` count.

| delivered `GSD` | `W_floor` | what it means for this study |
|---|---|---|
| 0.25 m | 1.0 m | the absolute floor binds; the existence check censors below any road width |
| 0.51 m | 1.53 m | censors below the narrowest 작업임도 class |
| 1.0 m | 3.0 m | censors a class of real narrow forest roads, and `W_cleared` below 3 m is not measurable at all |
| 2.0 m | 6.0 m | **the tripwire, declared now.** `W_floor` equals the width in the claim under test |
| coarser than 2.0 m | above 6.0 m | the existence check cannot be used |

> **The tripwire, and its consequence, fixed before the imagery arrives.** If the
> delivered ground sample distance is 2.0 m or coarser, `W_floor` is at or above
> 6 m, which is the width in the claim under test. The existence check would then
> censor exactly at the threshold the hypothesis is about, and **the roads arm is
> not executable through the existence route**: gate E1 must then be satisfied by
> the edition test of item 2 above or not at all, and if it cannot be, section 3.4's
> bounding case fires and the roads arm is reported as not executable. This is
> written here so that a coarse delivery cannot later be accepted with a caveat.

**(b) `NOEXIST` and `WIDTH` join the exclusion-clustering readings of section 12.9.**
v0.2's clustering check listed `NOBURN`, `NOTMAIN`, `NODETECT`, `TIMING` and `SIDE`
and omitted the two exclusions that are by construction functions of the measured
covariate. Both are added, with the dangerous-direction bounding analysis attached to
them, because the bias direction is known in advance for these two: the exclusion
runs against narrow roads.

**(c) Gate E1's consequence text records the double duty.** In the bounding case the
edition test and the existence check are **one measurement doing double duty**: E1
can be satisfied by the existence check alone, but the existence check cannot
establish existence, only the visibility of a gap at whatever ground sample distance
arrives. One imagery failure, or one imagery vintage too coarse, takes the width
covariate, the error model, `kappa_v`, gate M1, the recoverability simulation, check
W0 and the existence check together. **A pass of E1 obtained by the existence check
alone is recorded and reported under that name**, at `gates.E1.route`, and never as a
plain pass.
...
`NOEXIST` check of item 4, which needs WJ-009 **and a ground sample distance finer
than the 2.0 m tripwire of section 3.4.1(a)**. If WJ-009 also does not clear, the
...
Write `t_crit` for the cutoff. Then:

- **`T >= t_crit`**: the front arrived from the `-n` side. That side is windward.
- **`T <= -t_crit`**: the front arrived from the `+n` side. That side is windward.
- **`|T| < t_crit`**: the side assignment fails and the segment is indeterminate
  with reason `SIDE`.

#### 5.1.2a The cutoff is a t quantile set from a declared false-assignment rate

**This replaces v0.2's `z_side` of 2.0 outright and is condition C2.** v0.2 put a
normal-scale number on a statistic that is not on the normal scale. `se(s)` is
`sqrt(n' Sigma_g n)` and `Sigma_g` scales with `sigma_r`, which is **estimated**
from the residuals of the same weighted plane fit. The plane
`t(x) = t0 + g . (x - x0)` carries three parameters in two dimensions, so the
residual degrees of freedom are

    nu = n_det - 3

and `T` is a Student's t statistic on `nu`, not a z statistic. A3 re-ran A6's
arithmetic of what the mismatch costs and it reproduces exactly:

| `n_det` | `nu` | P(`T` > 2.0) per tail | the rate 2.0 looks like it promises |
|---|---|---|---|
| 5 (the C4 minimum) | 2 | 0.0918 | 0.0228 |
| 6 | 3 | 0.0697 | 0.0228 |
| 8 | 5 | 0.0510 | 0.0228 |
| 12 (the old tier S-B minimum) | 9 | 0.0383 | 0.0228 |
| 25 | 22 | 0.0290 | 0.0228 |
| 40 | 37 | 0.0264 | 0.0228 |

At the C4 minimum the nominal cutoff delivers about four times the rate it appears
to promise, and **no value on v0.2's declared grid repairs it there**: at `nu` of 2
a cutoff of 2.5 still gives 0.0648 per tail and 1.5 gives 0.1362. The grid was on
the wrong axis, so what changes is the test and not the threshold.

> **The rule.** The cutoff is evaluated per segment at that segment's own realised
> residual degrees of freedom:
>
>     t_crit(nu, alpha) = t_{1 - alpha}( n_det - 3 )
>
> The pre-registered **per-tier one-sided false-assignment rate is `alpha` = 0.01**,
> with a declared sensitivity grid of **{0.005, 0.01, 0.025}**, stated in `alpha`
> and never in `z` (section 12.8). `nu` is recorded per segment and is a key path
> (section 22.2), so the cutoff a segment was judged at is recoverable from the
> artifact.

**What `alpha` is the rate of, stated exactly, because a loose statement here is
what produced the 2.0 in the first place.** Under a true projected gradient
`s > 0`, `T` follows a **noncentral** t on `nu` degrees of freedom with
noncentrality `delta = s / se(s)`, which by section 5.1.3 is about
`sin(theta) sqrt(n_det) / (2c)`. The probability that the rule assigns the
**opposite** side is `P(T <= -t_crit)`. That probability is bounded above by
`alpha` and attains `alpha` only in the limit `delta -> 0`. So:

> **`alpha` is the supremum over the null boundary of the per-segment probability
> that this rule names the wrong side.** It is not the probability that a segment
> is assigned a side, and it is not the probability that an assigned side is wrong.
> Those two are different quantities, both are reported, and section 5.1.5 works
> them out.

**Why 0.01 and not something smaller or larger.** The level trades one failure
against another and both failures run in the same direction, which is the thing
that decides it.

1. **A wrong side removes a hold** (section 5.1.5), so the wrong-side rate is a
   direct tax on the rare class. At the sample sizes section 11.1 plans, with the
   held count in the low tens and gate G1 wanting at least 30 of them, the union
   bound of 0.02 across the two tiers puts the expected number of holds destroyed
   by side error below one at the G1 threshold and near one at twice it. That is
   below the three-in-thirty tolerance gate L1 already accepts for label-rule
   disagreement, which is the closest declared comparator this design has.
2. **A smaller `alpha` does not make the leak go away, it moves it.** Every segment
   that a stricter cutoff stops from being mislabelled is a segment that instead
   exits `SIDE`, and the `SIDE` exclusion is itself outcome dependent in the same
   hold-removing direction (section 12.9.1, condition C3). Driving `alpha` to
   0.001 would trade a bounded and countable error for an unbounded and
   harder-to-see exclusion. The two are one leak through two doors and `alpha` sets
   where it comes through.
3. **0.01 keeps the change a correction of scale rather than a covert tightening.**
   The 2.0 of v0.2 was evidently meant to buy about 0.023 per tail. A union bound
   of 0.02 over two tiers lands within a rounding of that intent, at the right
   sampling scale. Anyone reading v0.2 and v0.3 side by side should see the same
   ambition and a corrected instrument, not a new ambition smuggled in with a fix.

**What the correction costs, said before the table in 5.1.3 says it.** `t_crit` at
`alpha` = 0.01 is 6.97 at `nu` of 2 against the 2.0 it replaces, so the minimum
resolvable approach angle rises steeply at low detection counts and the low end of
the `n_det` range stops contributing. That is a real loss of sample and it makes
gate S1 more likely to fire. It is the design failing safe: the segments being lost
are the ones v0.2 would have assigned at a wrong-side rate four times its own
stated one.

#### 5.1.2b The two tiers are one two-stage procedure and are budgeted as one

**Tier S-B, the coarser fallback, applied only where tier S-A fails.** The same
...
A longer lever arm buys a smaller gradient standard error, at the cost of averaging
over terrain the front did not treat uniformly.

**A6 found, and A3 accepts, that v0.2 reported a two-stage test at a one-stage
threshold.** S-B is a conditional second look at a correlated statistic on an
enlarged neighbourhood, applied exactly where the first look failed, so the
realised wrong-side rate over the two stages exceeds either stage's nominal rate.
The repair is a declared budget rather than a footnote:

> **The pre-registered union target is a wrong-side rate of 0.02 per segment across
> both tiers.** It is split by Bonferroni into `alpha_A` = `alpha_B` = 0.01. The
> union bound holds because
> `P(wrong) = P(wrong at S-A) + P(S-A indeterminate and wrong at S-B)`, and the
> second term carries a factor below one, so 0.02 is conservative rather than
> exact. The grid of {0.005, 0.01, 0.025} moves both tiers together, so the union
> target moves with it to {0.01, 0.02, 0.05}.

Check S0 reports the S-A pass fraction, the S-B-given-S-A-failure pass fraction and
the union pass fraction, at every value of the grid, so **the procedure is reported
at its own operating characteristic**. Those are three key paths and not one.

**The third thing tier S-B introduces, which v0.2 did not name and A6 did.** At
3 km a detection cloud around a barrier is informative about the outcome by a route
that is not the burned mask: a 3 km neighbourhood spans a long barrier, so if the
front crossed, detections straddle the line, and if it held they sit on one side.
The geometry the S-B plane is fitted to is therefore partly a picture of the event
the label reads, observed by a different sensor. This does not break the letter of
the section 6.1 provenance firewall, which governs the scene pair and names the
detection record as a declared and legitimate input to the side rule. It does make
the S-B tier's behaviour outcome dependent, which is why the S-A-only secondary
below is load-bearing rather than decorative, and why `side_tier` is in the
12.9.1 stratification.

The tier that assigned each segment is recorded as `side_tier` in
{`S-A`, `S-B`, `none`}, it enters the model as a covariate, and the label rate by
tier is reported. **A pre-registered secondary refits on S-A segments only,
unconditionally**, rather than only if the label rate differs materially between
the tiers. v0.2 made that refit conditional on a judgement about materiality made
after seeing the rates, which is a degree of freedom this document has no reason to
keep; the refit is cheap and it is now always run and always reported.

#### 5.1.2c Two acquisition epochs is a different instrument, and it is separated

**The second half of condition C2.** At exactly two distinct acquisition epochs the
response `t` takes two values, so the fitted plane is separating two level sets
rather than tracking a propagating front. The residual then measures how planar two
clusters of detections are, which is within-epoch line thickness, rather than how
far the real front departs from a plane over the time it crosses the neighbourhood,
which is what `sigma_r` is defined to be in section 5.1.3. The between-epoch
curvature that `sigma_r` is supposed to see is not observed at all, because there is
no third epoch to reveal it. So `sigma_r` is biased low, `se(s)` is biased low and
`T` is biased high, **on top of** the t-versus-normal error, and in an amount no
argument in this document can quantify.

The bias cannot be corrected by a stricter cutoff, because nothing here says how
much stricter. What can be done honestly is to separate the stratum:

> **Tier S-A requires at least three distinct acquisition epochs.** A segment whose
> 1 km fit has exactly two epochs is not assigned a side by tier S-A. It falls
> through to tier S-B, which already requires three epochs, and if S-B cannot be
> fitted it exits `SIDE` with the sub-code **`SIDE_2EP`**, counted separately.
>
> **A pre-registered secondary admits the two-epoch segments** at the same `alpha`,
> with `n_epochs` carried as a covariate and with the bias direction stated beside
> it: `T` is biased high, so more segments are assigned and the realised wrong-side
> rate is above the nominal `alpha` by an unquantified amount. The primary is the
> three-epoch set. The comparison between the two is reported whichever way it comes
> out, and **which of the two is the headline is fixed here, before either count
> exists.**

Three consequences worth stating in the same breath, because each of them is a cost
this choice creates rather than removes:

- **It raises the side rule's effective detection requirement.** The C4 minimum of 5
  detections stands, but 5 detections spanning 3 epochs is a thinner cloud per epoch
  than 5 spanning 2. Check S0 reports the joint distribution of `n_det` and
  `n_epochs` per fire, and the fraction of candidates lost to the three-epoch
  requirement at each value of the C4 epoch grid {2, 3}. **The cost is measured
  before labelling, not assumed.**
- **`SIDE_2EP` is an exclusion like any other**, so it joins the section 12.9
  clustering readings and the section 12.9.1 side-invariant check. An exclusion
  introduced to fix a bias is still an exclusion and is audited as one.
- **It interacts with the overpass confound of section 2.3.** NOAA-21 covers only
  the 2025 fires, so the epoch count per segment is partly a property of the fire,
  and the fire is the fold unit and the random-effect level. The three-epoch
  requirement therefore removes segments unevenly across folds. That is reported per
  fire in check S0 and it is the same door section 12.7 is now told to watch (C4).
...
the neighbourhood at all. Requiring `T >= t_crit` gives the operational condition

    sin(theta)  >=  2 * t_crit(n_det - 3, alpha) * c / sqrt(n_det)

which is a statement about approach angle, detection count and front irregularity,
and about nothing else. v0.2 wrote the right-hand side as `4c / sqrt(n_det)`, which
is this expression with `t_crit` frozen at 2.0; condition C2 unfreezes it.

**The minimum resolvable approach angle, in degrees, recomputed at `alpha` = 0.01.**
`none` means the requirement exceeds `sin(theta) = 1`, so no approach angle resolves
a side at that combination and every such segment exits `SIDE` whatever the geometry.

| `c` | `n_det` = 5 | 6 | 8 | 12 | 25 | 40 |
|---|---|---|---|---|---|---|
| `t_crit`, `alpha` = 0.01 | 6.965 | 4.541 | 3.365 | 2.821 | 2.508 | 2.431 |
| 0.15 | 69.1 | 33.8 | 20.9 | 14.1 | 8.7 | 6.6 |
| 0.20 | none | 47.9 | 28.4 | 19.0 | 11.6 | 8.8 |
| 0.30 | none | none | 45.5 | 29.3 | 17.5 | 13.3 |
| 0.40 | none | none | 72.1 | 40.7 | 23.7 | 17.9 |
| 0.50 | none | none | none | 54.5 | 30.1 | 22.6 |

The same table read the other way, which is the more useful reading and is new:
**the largest front irregularity at which a side is resolvable at any approach
angle at all**, `c_max = sqrt(n_det) / (2 t_crit)`.

| `n_det` | 5 | 6 | 8 | 12 | 25 | 40 | 60 |
|---|---|---|---|---|---|---|---|
| `c_max` at `alpha` = 0.01 | 0.16 | 0.27 | 0.42 | 0.61 | 1.00 | 1.30 | 1.62 |

**Compare against v0.2 and say the cost plainly.** Under the 2.0 cutoff the same
corner read 15.6 degrees at `n_det` of 5 and `c` of 0.15. It now reads 69.1
degrees, and at `c` of 0.20 it does not read at all. **The bottom of the detection
range has stopped contributing**, and that is not a tightening chosen for
conservatism; it is what the stated false-assignment rate was always worth at two
residual degrees of freedom. The design was previously buying that corner at about
four times the wrong-side rate it believed it was paying.
...
is a pre-registered pre-labelling check. Check S0 is specified here in the form
condition C2 requires, and it is implemented at
`research/roads/s0_side_check.py` with its tests, which run on synthetic geometry
only and are marked as such.

> **Check S0, run before any label is computed.** Fit the section 6.4 plane at
> every candidate segment on all five fires and record, per segment:
> `sigma_r`, `n_det`, **`n_epochs`**, **`nu = n_det - 3`**, `R`, the fitted `r`,
> the approach angle `theta`, the statistic `T`, the tier, and the per-tier
> `t_crit` the segment was judged at. Form `c = sigma_r r / R` per segment and
> report its distribution per fire and per barrier type.
>
> From the same run, and **at every value of the `alpha` grid {0.005, 0.01, 0.025}**,
> report:
>
> 1. the **tier S-A pass fraction**;
> 2. the **tier S-B pass fraction given an S-A failure**;
> 3. the **union pass fraction**, which is the one gate S1 reads;
> 4. the realised degrees of freedom distribution, and the joint distribution of
>    `n_det` and `n_epochs`;
> 5. the fraction of candidates lost to the three-epoch requirement of 5.1.2c, at
>    each value of the C4 epoch grid {2, 3};
> 6. the **plug-in wrong-side diagnostic** of section 5.1.5, with its stated bias
>    direction;
> 7. the per-fire breakdown of every one of the above, because the fire is the fold
>    unit.
>
> This is a function of the detection record and the barrier geometry only; it
> touches no scene and no label, and it is committed before labelling starts.

At `n_det` of 12 to 25 the rule resolves most of the angular range for `c` up to
about 0.3, which is the working regime this design needs. **If check S0 reports `c`
above about 0.4 at typical detection counts, the side assignment fails for most
segments and this design cannot produce a label**, and that is known before the
labelling effort is spent rather than after. v0.2 put that cliff at about 0.5; the
corrected cutoff moves it to about 0.4 at `n_det` of 8 and leaves it near 0.6 at 12.
Gate **S1** in section 11.3 is the rule that fires on it, and it is now stated on
the union pass fraction rather than on a single tier.

#### 5.1.3a What this check cannot tell us, and is not allowed to be read as

`c` is estimated from a plane fit whose residual is the same residual the side test
divides by, so a segment with an unluckily small `sigma_r` gets both a small `c` and
a large `T`. The pass fraction check S0 reports is therefore **not** an unbiased
forecast of the pass fraction the labelling run will see; it is the realised pass
fraction of the same procedure on the same data, which is what gate S1 needs and is
the reason gate S1 reads the pass fraction directly rather than reading `c`.

`c` is also weakly determined at the bottom of the range. `r` is the reciprocal of
the fitted gradient magnitude, and with few epochs that is poorly determined
(section 6.4), so `c` inherits the noise. A6 makes the point that repairs this and
A3 adopts it: **`c` is not in the critical path of the label.** The side test is
`T = s / se(s)`, computed directly from `Sigma_g`; `c` appears only in this
section's derivation and in check S0's diagnostic distribution. A poorly estimated
`c` therefore mislabels no segment. What it misestimates is the forecast of how many
segments survive, and gate S1 is already written against the realised pass fraction
rather than against `c`.

#### 5.1.4 The grazing truncation, which is not benign, and reaches `kappa_v`
...
**v0.2 declared this and then read its own cost too kindly. A6 is right and A3
withdraws v0.2's characterisation.** The truncation reaches `kappa_v`, which is
what gate M1 gates on, by three mechanisms that interact and whose net sign cannot
be settled by argument. They are set out here because naming them is what makes the
net sign a computable quantity rather than a matter of opinion.

**Mechanism 1, the numerator, and it lowers `kappa_v`.**
`kappa_v = Var(v_true) / (Var(v_true) + mean_i sigma_v_i^2)` with
`v = log(W_eff + 1)`. The side rule truncates `W_eff` from above through
`sin(theta)`. Truncating the top of the range lowers `Var(v_true)`, which lowers
`kappa_v`. Section 11.2 already knows that a narrow spread of true width is what
...
same quantity**, and v0.2 connected the two nowhere.

**Mechanism 2, the denominator, and it raises `kappa_v`.** Section 11.5.1 has
`sigma_v_i` about `sigma_u / (W_cleared_i + s_i)`, largest for narrow barriers at
grazing angles. The side rule removes grazing angles, so it removes the
worst-measured segments, which raises `kappa_v`. **Mechanisms 1 and 2 pull opposite
ways**, so the net effect on `kappa_v` is arithmetic and not argument. The
arithmetic is available before any label exists, from check S0's per-segment `theta`
and pass flag together with the width measurement, which section 12.9 already
establishes is made for every candidate segment whether or not it is ever labelled.
Section 11.5.1 now computes `kappa_v` on both geometries for exactly this reason
(condition C4).

**Mechanism 3, and this one is not symmetric.** On the retained set the rule
enforces `sin(theta) >= 2 t_crit c / sqrt(n_det)`, so

    W_eff  <=  W_cleared * sqrt(n_det) / ( 2 * t_crit(n_det - 3, alpha) * c )

**The ceiling on the width covariate is a function of the detection count and of
the cutoff.** `n_det` rises with overpass cadence, which is section 2.3's own
declared confound: Suomi NPP and NOAA-20 cover all five fires, NOAA-21 only the
2025 ones. So the retained width support differs by fire, and the fire is also the
fold unit and the random-effect level. That is section 12.7's collinearity between
the complex intercept and the width slope arriving through a door section 12.7 did
not watch, because 12.7 reported within-complex width variation with nothing saying
it was reported on the retained set. It is now (condition C4). Note that the
correction of C2 **raises** `t_crit` and therefore **lowers** the ceiling, so
mechanism 3 is stronger in v0.3 than it was in v0.2, and stronger at low `n_det`
than at high, which is precisely where the cadence confound lives.

**Mechanism 4, which is v0.2's own and stands.** The 15 degree clip in `W_eff` is
kept unchanged. Under this rule the clip is rarely binding, since the side test
already requires a larger angle in most configurations, and a clip that almost never
binds is preferable to one tuned after the fact.

**The compounding at the other end, which is condition C5.** In the bounding case of
section 3.4 the roads arm survives only through the `NOEXIST` per-segment existence
check, and that check is the width measurement's own null result, so it truncates
`W_cleared` from **below**. Combine the two and the roads width support is squeezed
from below by the instrument that measures width and from above by the rule that
assigns sides, both through the covariate's own machinery, with `Var(v_true)`
between them. A3 accepts A6's statement of this and adds one qualification, in
section 17.5, about which gate it actually bears on.

**What is pre-registered in response.** Four things, of which only the second
survives unchanged from v0.2:
...
1. The `SIDE`-excluded segments' `W_cleared` and approach-angle distributions are
   ...
   clustering check of section 12.9, now with `SIDE_2EP`, `NOEXIST` and `WIDTH`
   joining the codes.
2. The width slope is reported on **`W_cleared`** as well as on `W_eff`, and F3 and
   the smallest effect of interest are stated in `W_cleared` (sections 1.5, 11.5.3).
   **v0.2 said this meant the headline quantity was the one the truncation does not
   act on directly. That sentence is withdrawn.** It is true of the filter and false
   of the model: the fitted covariate is still `v`, gate M1 still reads `kappa_v`
   which is defined on `v`, and F3 is a contrast read off the fitted curve rather
   than off a separate `W_cleared` regression. Reporting a `W_cleared` slope beside
   it is worth doing and does not remove the interaction.
3. `Var(v_true)`, `kappa_v` and `kappa_v_i` are computed on **both** the R-set of
   section 5.1.6 and the full candidate set, both reported at distinct key paths,
   with gate M1 reading the R-set value (condition C4).
4. The three mechanisms above are reported as three numbers rather than as prose:
   the candidate-set and R-set `Var(v_true)`, the candidate-set and R-set
   `mean_i sigma_v_i^2`, and the realised `W_eff` ceiling per fire. A reader can
   then see which mechanism won, which is the whole content of A6's finding.

#### 5.1.5 What a wrong side assignment does, and which way it runs

**A6 raises this as the reason C2 is blocking rather than cosmetic, and A3 agrees
with the direction and works out the size.** Nothing here is a measurement; it is
the section 5.7 procedure applied to a swapped segment, plus arithmetic.

**The mechanism.** A false side assignment swaps the two bands. Section 5.7
evaluates the side test at step 5 and C1 at step 8, so a swapped segment has C1, the
windward-burned-up-to-the-barrier test, applied to what is really the lee band.

- **On a crossed segment the swap is harmless.** Both bands burned, so C1 passes on
  either band, and `f_lee` computed on the true windward band is high, so the
  segment still exits `CROSSED` at step 12.
- **On a held segment the swap is fatal.** The true windward burned and the true lee
  did not, so C1 applied to the true lee fails, and the segment exits **`NOBURN`**,
  ineligible.

> **Side assignment error removes holds and leaves crossings.** It acts only on the
> rare class, which section 11.1 already expects in the low tens and which gate G1
> wants at least 30 of. This is leakage item A10 and B3 arriving through the side
> rule, and it is why `alpha` is set from this consequence in section 5.1.2a rather
> than from a round number.

**What it does to the held-versus-crossed ratio.** Let `q` be the realised side-test
pass fraction that check S0 reports and gate S1 reads, and let `w_i = P(T_i <= -t_crit)`
be the per-segment wrong-side probability, bounded by `alpha`. Assume for the moment
that the noncentrality distribution is the same for holds and for crossings; the
paragraph after this one removes that assumption. Then retained holds are
proportional to `E[a(1 - w)]` and retained crossings to `E[a]`, where `a` is the
per-segment assignment probability, and

    R_observed / R_true  =  1  -  E[w] / E[a]  >=  1 - alpha / q

**The numbers this gives, at the union budget of section 5.1.2b and the pass
fraction gate S1 already requires.** Gate S1 needs `q` at or above 0.25, so:

| cutoff regime | per-tail wrong-side rate | worst-case depression of the held:crossed ratio at `q` = 0.25 |
|---|---|---|
| v0.3, one tier, `alpha` = 0.01 | 0.01 | 0.04 |
| v0.3, both tiers, union bound | 0.02 | 0.08 |
| v0.2's 2.0 at `nu` of 2, one tier | 0.0918 | 0.37 |
| v0.2's 2.0 at `nu` of 2, both tiers | about 0.18 | at the bound, the ratio is not usable |

So the correction takes the worst case on the rare class from **about a third of the
held class** at the bottom of the detection range down to **under a tenth**, and the
realised figure is far smaller than the bound, because `w_i` collapses fast once the
true side is real at all. At `alpha` = 0.01 and a true noncentrality of 1.0 the
per-segment wrong-side probability is already 0.0015 at `nu` of 2 and 0.0005 at
`nu` of 22; at a noncentrality of 2.0 it is below 0.0001 everywhere. **The bound is
attained only where there is nothing to get right**, and those segments mostly exit
`SIDE` instead. This is the same observation as the `alpha` trade in 5.1.2a, seen
from the other side.

**The plug-in diagnostic check S0 reports, and its declared bias.** Check S0 holds
`T_i` per segment, which is a noisy estimate of `delta_i`, so a plug-in wrong-side
rate `sum_i P(T <= -t_crit | delta = |T_i|) / sum_i 1{|T_i| >= t_crit}` is
computable before any label. It is reported as a diagnostic with its bias direction
stated: `|T_i|` overstates `|delta_i|` on the retained set, because retention
selects on `|T|`, so the plug-in **understates** the wrong-side rate. No correction
for that is pre-registered, because a shrinkage estimator chosen now would be one
more thing to choose. The bound of `alpha / q` is the number that is allowed to be
quoted as a bound; the plug-in is allowed to be quoted only as a diagnostic with
this sentence attached.

**Does it bias the breach curve?** Three separate answers, because they differ.

1. **The level moves and the movement is not small.** Holds are removed and
   crossings are not, so the fitted intercept shifts toward a higher breach
   probability at every width. In a logistic model, sampling that depends on the
   outcome alone moves the intercept and leaves the slope alone, so this is the
   part that is both largest and least damaging to the coefficient of interest. It
   is reported: the label accounting carries the counts, and the bounding analysis
   of section 10.5 already enters excluded segments both ways.
2. **The slope in `W_cleared` is unbiased to first order, conditional on one thing
   that is checkable before labelling.** `w_i` is a function of `delta_i`, hence of
   `theta_i`, `n_det_i` and `c_i`. None of those is `W_cleared`. So unless approach
   angle is associated with cleared width, the loss is non-differential in
   `W_cleared` and F3, which is stated in `W_cleared`, is protected. **Whether that
   association exists is measured, not assumed**: the rank correlation between
   `theta` and `W_cleared` on the candidate set is reported by check S0 at
   `checks.S0.theta_vs_wcleared_rho`, before any label exists, and the reading is
   written here in advance. A correlation materially different from zero means the
   protection in this item does not hold and the item is reported as not holding.
3. **The slope in `v = log(W_eff + 1)` is attenuated, which is the conservative
   direction.** `w_i` rises as `sin(theta_i)` falls, and `W_eff_i` rises as
   `sin(theta_i)` falls, so at fixed `W_cleared` the wrong-side probability is
   increasing in `W_eff`. Holds are therefore lost preferentially at the wide end of
   the fitted covariate, which pulls the width slope toward zero. Under `H-ROADS`
   that is attenuation and not inflation, so this route cannot manufacture the
   result the hypothesis predicts.

**The one residual, and it is real.** The assumption in the ratio arithmetic above,
that the noncentrality distribution is the same for holds and for crossings, is
false in the direction section 12.9.1 exists to measure: a stopped front leaves a
kinked, one-sided arrival surface, which is a larger `c`, which is a smaller
`delta`, which is both a higher `w_i` and a higher exclusion rate. So holds are
hit harder than the shared-distribution arithmetic says, by an amount this document
cannot bound. **If that excess is itself associated with width, the loss becomes
differential and item 2 above stops protecting the slope.** That is exactly what the
width-tercile stratification in section 12.9.1 measures, and it is the reason C3's
check is stratified by width rather than pooled.

#### 5.1.6 The R-set, which is the geometry the design's instruments run on

**This is condition C4 and it is a definition rather than a rule, but it has to be
here rather than in section 11, because it is a property of the side test.** v0.2
said instruments 2 and 2b take "the **real** segment geometry and the **real**
measured width distribution" and never said which real set that is. Both the
candidate set and the side-passing set are real, both exist before labelling, and
they have different sizes and different width supports. A power figure computed on
the candidate set overstates the power of a fit that happens on the smaller and
truncated one.

> **The R-set** is the set of candidate segments that pass steps 1, 2 and 5 of the
> section 5.7 procedure: barrier dating or existence, width measurable, and side
> assigned in either tier. It is everything the design can determine **before any
> scene is read**, and it is committed as a file with a row count when check S0 and
> the width measurement have both run, before labelling starts.

Three sets, three uses, fixed here:

| set | when it exists | what is computed on it |
|---|---|---|
| **candidate set** | after the layout script | `Var(v_true)`, `kappa_v`, `kappa_v_i` reported for comparison; the section 12.9 exclusion denominators |
| **R-set** | after check S0 and the width measurement | `Var(v_true)`, `kappa_v`, `kappa_v_i` **that gate M1 reads**; instruments 2 and 2b; **gate R0**; check W0; section 12.7's within-complex width variation |
| **labelled set** | after labelling | the fit. `kappa_v` recomputed and reported a third time, as the diagnostic below |

**The residual A3 adds rather than hides.** The R-set is still a **superset** of the
labelled set: steps 3, 4, 6, 7 and 8 of section 5.7 remove more segments after the
scenes are read, and section 12.9 already expects that removal to be outcome
dependent. So an instrument computed on the R-set still overstates the power of the
fit that actually runs, by less than one computed on the candidate set and by more
than zero. The gates have to fire before labelling, so they cannot read the labelled
set, and moving them later would defeat the purpose of gate R0. What is done instead:

> `Var(v_true)`, `kappa_v` and detection power are **recomputed on the labelled set
> after labelling and reported beside the R-set values**, at their own key paths, as
> a measurement of how much the pre-labelling instruments overstated. They change no
> gate and they are reported whichever way they come out. Gate M1 reads the R-set
> value, as condition C4 requires, and the labelled-set value is reported next to it
> so that a reader is never shown only the more favourable of the two.
...
| `SIDE` | the projected arrival-time gradient does not resolve windward from lee at `t_crit(n_det - 3, alpha)` in either tier | 5.7 step 5 |
| `SIDE_2EP` | the 1 km fit had exactly two distinct acquisition epochs, so tier S-A does not apply (5.1.2c), and tier S-B could not be fitted | 5.7 step 5 |
...
>
> **Step A records the time margin it is implicitly asserting. This is condition
> C6 and it changes no label.** A distance threshold is the right shape only where
> the time margin it implies exceeds the timing uncertainty, and this document holds
> both quantities per segment and v0.2 never compared them. So for every segment
> passed on geometry alone, the pipeline computes the implied flanking delay
> `d_bypass / r_flank`, using 2,000 m when `d_bypass` is undefined because that is
> the search window, and compares it against the combined standard error of `t_w`
> and the acquisition time. The **count of geometric passes whose implied delay
> falls below that standard error** is reported per fire at
> `labels.crossed_reachability.by_geometry_thin_margin`, together with the margin's
> distribution in hours. It is reported whichever way it comes out.
>
...
- every segment with `d_bypass` undefined or above 500 m reaches `CROSSED` on
  ...
  **and A6 is right that this is a cost as well as a repair.** If most `CROSSED`
  labels are reached on geometry alone, the attribution test performs no attribution
  on most of the positive class, and the whole guard against flanking rests on 500 m
  being far enough. At a flanking spread rate of 1 km/h a bypass point just past
  500 m is about half an hour away, and section 12.11 reports that the fitted
  arrival standard error `se_t` exceeds two hours on some segments and counts them.
  **The size of the unchecked class is therefore measured rather than argued**,
  which is condition C6 and is the step-A paragraph above;
...
| 5 | the side test of section 5.1.2 fails in both tiers at `t_crit(n_det - 3, alpha)` | **indeterminate** `SIDE`, or `SIDE_2EP` when the 1 km fit had exactly two acquisition epochs and tier S-B could not be fitted (5.1.2c) |
...
implicit. **Section 5.1.5 is the reason that ordering matters more than it looks**:
because step 5 precedes step 8, a wrong side turns a held segment into a `NOBURN`
exit rather than into a mislabelled row, so the side rule's error does not
contaminate the labelled table, it thins the rare class inside it. Step 6 needs a side, which is why it sits after step 5 although it is an
...
| `n_epochs` | detection record, section 5.1.2c | arrival hour | no | count of distinct acquisition epochs in the 1 km fit; enters only the two-epoch secondary of 5.1.2c |
...
|---|---|---|---|
| **C1** | before anything else | every dataset that supplies a barrier, a covariate, a scene, a detection or a weather value is at status `verified` with a sha256 in `research/data/REGISTRY.yaml` | **no fit runs on real labels.** This is condition C1 of the sign-off record and it is not A3's to satisfy. It is written as a gate rather than as prose so that it has a boolean in the artifact like every other precondition, and so that a run cannot proceed past it by inattention. Section 21.1 holds the current status of all ten |
| **E1** | after acquisition, before layout | at least two fire complexes supply road segments that pass the edition or existence check of section 3.4 | the roads arm is not executable. Rivers and ridges continue, the width slope is reported for rivers only, and no statement of any kind is made about forest roads. **The route by which E1 passed is recorded at `gates.E1.route`** (condition C5(c)): in the bounding case of section 3.4 the edition test and the existence check are one measurement doing double duty, so a pass obtained by the existence check alone is reported under that name and carries the ground sample distance and the `W_floor` of section 3.4.1(a) beside it |
| **S1** | after detections land, before labelling | check S0 of section 5.1.3 shows at least 0.25 of candidate segments assigned a side, **counted on the union of tiers S-A and S-B at `alpha` = 0.01 per tier**, in at least three complexes | the side assignment fails at scale. The round delivers check S0's distributions as its result and no label is computed, because a labelled table built from a quarter of a quarter of the candidates is not a table. **Restated in `alpha` by condition C2**, and on the union rather than on one tier, because the two tiers are one two-stage procedure (5.1.2b). The pass fraction at each value of the `alpha` grid is reported whichever way the gate goes |
| **W0** | after width measurement, before labelling. **Not a gate: a projection** | on the R-set of section 5.1.6, within the road barrier type, the counts of segments with `W_cleared` at or above 6 m and below 6 m, and the R-set `kappa_v` | nothing. W0 fires no branch. It exists because gate W1 and gate M1 both read quantities whose pre-labelling upper bounds are already knowable, and A6 and A3 disagree about which of the two binds first (section 17.5). W0's counts are an **upper bound** on W1's, since labelling only removes segments, so W0 below 25 on either side of 6 m means W1 will fail, while W0 above it implies nothing. Reported whichever way it comes out, so the ordering is measured instead of predicted |
| **R0** | after width measurement, before labelling | instrument 2 of section 11.5.2, **computed on the R-set of section 5.1.6**, reports detection power at the smallest effect of interest of at least **0.20** | the labelling proceeds for the descriptive deliverables only and **the width fit is not attempted**. 0.20 is a much lower bar than the 0.80 of instrument 2 and it is a different decision: 0.80 decides whether a flat curve is informative, 0.20 decides whether the labelling effort is worth spending on the width question at all. **Condition C4 fixes the geometry**: R0 reads the R-set value, the candidate-set value is reported beside it, and the two are at distinct key paths |
...
| **M1** | after the fit | the **lower end of the 90 per cent posterior interval** of `kappa_v` (section 11.5.1), **computed on the R-set of section 5.1.6**, is at or above **0.70**, and at least 60 hand-measured segments support the error model (section 6.3.3) | the width covariate is declared too noisy to carry the question: no breach curve against width is published, F1 cannot be reported as evidence either way, and the outcome is written as **not resolvable at the achieved measurement precision** |
...
**And because W1 counts in `W_cleared`, the two truncations of section 5.1.4 do not
both reach it.** The side rule truncates `W_eff` from above, through `sin(theta)`,
and `theta` is a property of the front and the barrier's bearing rather than of the
road's design class. It reaches W1 only through whatever association exists between
`theta` and `W_cleared`, which check S0 measures before any label
(`checks.S0.theta_vs_wcleared_rho`). The existence check truncates `W_cleared`
itself, from below, and reaches W1 directly. So W1 is squeezed from below, and it is
`kappa_v` and therefore gate M1 that is squeezed from both ends. A3 records this in
section 17.5 as a qualification of A6's finding rather than a disagreement with it,
and check W0 above is what settles the ordering with a number.
...
**Which sample, and this is condition C4.** "Across the sample" was undefined in
v0.2 and the answer changes the number. `Var(v_true)`, `kappa_v` and `kappa_v_i` are
computed on **the R-set of section 5.1.6**, which is the geometry that will be
fitted, and **additionally on the full candidate set**, which is the geometry v0.2's
wording would have let a reader assume. Both are reported, at distinct key paths,
and **gate M1 reads the R-set value**. After labelling, a third value is computed on
the labelled set and reported beside them, gating nothing, as the measurement of how
much the pre-labelling instruments overstated (section 5.1.6). Reporting one of
these three without the others is forbidden by section 22.2, which is why all three
have key paths and none of them is the artifact's only `kappa_v`.

- **`kappa_v` is the single summary that gate M1 gates on**, and the document says
  which summary it is: the sample-mean-error version above, on the R-set, reported
  with a 90 per cent posterior interval, with M1 reading the lower end.
...
- **The three mechanisms of section 5.1.4 are reported as numbers.** The
  candidate-set and R-set `Var(v_true)` (mechanism 1), the candidate-set and R-set
  `mean_i sigma_v_i^2` (mechanism 2), and the realised `W_eff` ceiling per fire
  (mechanism 3). The net sign of the grazing truncation on `kappa_v` is then read off
  three pairs of numbers rather than argued, which is the whole content of A6's
  finding and is available before any label exists.
- **A6's addition to the M1 arithmetic, recorded because it makes the cost larger.**
  The derivation in section 11.3 that turns a 0.70 interval bound into a point
  requirement near 0.77 treats `Var(v_true)` as known. It is estimated, from the same
  sample, on a support that two of this design's own filters truncate. **So 0.77 is a
  floor on the point requirement and not the requirement**, and it is written that
...
Take **the R-set geometry of section 5.1.6** and **the R-set measured width
distribution**, including its real measurement error from instrument 1. **Condition
C4 pins this and v0.2 left it open**: "the real segment geometry" was true of both
the candidate set and the side-passing set, and a power figure computed on the
candidate set overstates the power of a fit that happens on the smaller, truncated
one. Instruments 2 and 2b are run on the R-set, gate R0 reads the R-set value, and
the candidate-set value is computed and reported beside it so the size of the
overstatement is visible. `recoverability.*` records which geometry each number came
from, at `recoverability.geometry`. Simulate outcomes under a
...
**The refit backend is pre-registered, because a power number is a property of the
fit that produced it.** The harness is committed at
`research/roads/recoverability.py` with its tests, and it exposes the refit as a
named backend so the harness can be exercised before any real geometry exists
without a fast approximation being mistaken for the pre-registered number:

| backend | what it is | may gate R0 or M2 read it |
|---|---|---|
| `nuts` | **the pre-registered one.** PyMC with NUTS at the section 22.4 settings, the same model form and the same errors-in-variables layer the real fit uses | **yes** |
| `laplace` | a MAP fit with a Laplace posterior and regression calibration in place of an integrated latent covariate. Seconds rather than minutes | **no**, never |

`recoverability.backend` is a key path and a `laplace` value carries
`gate_readable: false`. A `laplace` number may be reported as a harness diagnostic
with that label attached, and may not be read by any gate, in the same way
`kappa_raw` is reported and gates nothing (11.5.1). The section 22.4 compute cut
that drops instruments 2 and 2b from 500 draws to 200 is a reduction in draws and
never a substitution of backend.

**One thing the harness surfaced that belongs here rather than in the code.** The
probability contrast between 3 m and 6 m of `W_cleared` is bounded above at any
slope, because `v(3 m)` and `v(6 m)` are close on the log scale, so past some
magnitude a larger slope saturates both probabilities and the contrast falls again.
The harness raises rather than clipping when a requested effect is unreachable at
the simulation's intercept. **If the smallest effect of interest turns out to be
unreachable at the fitted intercept, that is a reportable design finding and not a
harness error**, and it is reported as one: the largest attainable contrast is
written to `recoverability.max_attainable_contrast` beside the requested one.
...
The second arm, at the same 500 draws, **the same R-set geometry of section
5.1.6**, and the same real measurement error:
...
**RC-011 has landed, and the remaining gap is condition C7.** The orchestrator
validated RC-011 in both directions and added it to `research/FORBIDDEN_CLAIMS.md`,
where it is now rule 11 of 11 with its detector in the program checker. A3 drafted
it and could not land it, because A3 does not own that file, and that sequence is
the one the program wants repeated: the agent that needs a rule drafts it with its
catches and its spares, and the owner validates and lands it.

What neither detector reaches is anything outside `research/`. The program checker's
scope is `research/`; `check_vocabulary.py`'s scope is `research/roads/` and
`research/reports/A3/`. A width sentence rendered into a poster caption, a README
line or the manuscript under `docs/` is covered by neither, and the drafted patch to
`scripts/check_forbidden.py` is on the taskboard as **WJ-002**, which is a human
gate. So, as a standing constraint of this pre-registration:

> **Until WJ-002 lands, no width number and no width sentence from this direction
> appears on any surface outside `research/`.** Not in `docs/`, not in the README,
> not in the manuscript, not in a poster, not in judge Q&A material. This is
> condition C7, it is blocking for the named secondary claim that the estimand is
> operational barrier performance, and it binds whether or not a number has been
> registered, because registration and phrasing are different checks.

**The record class, which is the other half of condition C7.** The sign-off record's
section 7 asks for two narrowings of the frozen-record construction A3 proposed.

1. **Membership granted by path to files that do not exist.** A6 caught this and
   **it is already fixed**, by the orchestrator, inside
   `research/shared/check_research_claims.py`: `RECORD_CLASS` now names the roads
   `PREREGISTRATION.md` alone, carries the rule that a path joins only once it is
   **actually** superseded and never in advance, and records why. A3 records the
   same rule for the detector it does own: `check_vocabulary.py`'s `FROZEN` tuple
   names files one at a time, never by pattern, and never in advance.
2. **Membership by path and not by content.** A6 fixed this inside its own tree by
   pinning the frozen file's sha256 in `research/eval/tests/test_signoff_roads.py`,
   so an edit to the one file that must never be edited now fails a test. A3 has
   nothing to add and does not duplicate the pin.

**And a decision this version has to make, because v0.2 is superseded today.**
`research/roads/PREREG_roads_2026-09-16_v0.2.md` is now frozen. It is **not** added
to either record class, because it passes both live detectors unedited, and adding a
file to a record class removes a detector from it. The record class exists for files
that cannot pass without being edited; a file that passes does not need it. The rule
A3 is following, and stating so that a later agent does not add it by reflex, is:
**a frozen file joins a record class only when the alternative is editing it.**
...
**On which set, and this is part of condition C4.** v0.2 reported within-complex
width variation and did not say it was reported on the retained geometry, which is
the door this section did not watch. The retained `W_eff` ceiling is
`W_cleared * sqrt(n_det) / (2 t_crit c)` by section 5.1.4 mechanism 3, and `n_det`
varies by fire with the overpass cadence of section 2.3, so **the retained width
support differs by fire and the fire is the fold unit**. Within-complex width
variation is therefore reported on the **R-set** as well as on the candidate set,
both per complex, and the difference between the two is itself reported. A complex
whose R-set width support collapses relative to its candidate support is a complex
whose intercept is absorbing width, and that is visible before any label.
...
| `alpha`, side test, per tier | 5.1.2a | 0.01 | {0.005, 0.01, 0.025} | how many segments survive the `SIDE` step, and the wrong-side rate of section 5.1.5. **Stated in `alpha`, never in `z`**, per condition C2. The union budget of section 5.1.2b moves with it to {0.01, 0.02, 0.05} |
| side-test minimum epochs, tier S-A | 5.1.2c | 3 | {2, 3} | `SIDE_2EP`. At 2 it is the pre-registered two-epoch secondary, reported with its bias direction, never as the primary |
...
3. **The `alpha` grid is a selection grid, not a labelling grid.** It changes which
   segments are labelled rather than how a labelled segment reads, so its output
   belongs in the accounting table and in the sections 12.9 and 12.9.1 checks, and
   its effect on the width posterior is reported there rather than as a label
   sensitivity. **It is not, however, only a selection grid**, which is the
   correction condition C2 forces: through section 5.1.5 it also sets the rate at
   which held segments are converted into `NOBURN` exits, so each value of `alpha`
   carries its own held-versus-crossed ratio and its own wrong-side bound
   `alpha_union / q`, and both are reported at every value.
4. **The `alpha` grid and the epoch grid are coupled.** Raising the epoch minimum
   from 2 to 3 removes segments whose `T` was biased high, so it interacts with the
   cutoff rather than acting beside it. The two are run one at a time from the
   primary like every other parameter, and the pair `(alpha, min_epochs)` at
   (0.01, 3) is the primary. The four corner combinations are **not** run, because a
   two-dimensional grid here would be a search rather than a sensitivity.
...
| C4 minimum distinct acquisition epochs | 4.4 | 2 | {2, 3}. The **side test** requires 3 regardless (5.1.2c); this row governs C4 eligibility |
...
1. the distribution of `W_cleared` among segments excluded at each of `NOBURN`,
   `NOTMAIN`, `NODETECT`, `TIMING`, `SIDE`, **`SIDE_2EP`, `NOEXIST` and `WIDTH`**,
   against the retained ones, with the mean difference and a cluster bootstrap by
   barrier line. **The last three are added by conditions C2 and C5.** `NOEXIST` and
   `WIDTH` are by construction functions of the measured covariate (section 3.4.1),
   so for those two the bias direction is known in advance and is the dangerous one,
   and the bounding analysis below is attached to them whichever way their
   coefficient comes out rather than only on the dangerous branch;
...
### 12.9.1 Whether the exclusions cluster on the outcome, measured without a side

**This is condition C3, it is the finding A6 rates highest in its whole record, and
A3 accepts it without reservation.** Section 12.9 checks exclusion against **width**.
It cannot check exclusion against the **outcome**, and A6 gives the obvious reason:
an excluded segment has no outcome, because the outcome needs a side.

**The mechanism, restated so the check is legible.** A front that crossed a barrier
cleanly leaves a locally planar arrival surface through the segment. A front that was
**stopped** at a barrier leaves a kinked, one-sided, locally non-planar arrival
surface at exactly that segment: arrival times pile up along the windward edge and
there is nothing on the other side. That is a large `sigma_r`, which is a large `c`,
which is a small `T`, which is `SIDE` indeterminate. **The side rule is less able to
resolve a hold than a crossing, and the failure removes the segment.** Same direction
as the C1 and C3 arrival censoring, same leakage item, a new route, and the design's
most important covariate is estimated on what survives. It is also the same mechanism
that puts the residual at the end of section 5.1.5, because a larger `c` is both a
higher wrong-side probability and a higher exclusion rate.

**The repair, which is A6's and is adopted verbatim in substance.** The outcome has a
**side-invariant** form. For any segment meeting the section 4.0 pixel minimum on
both bands, let `f_A` and `f_B` be the burned fractions of the two bands. Which is
windward and which is lee is exactly what an excluded segment does not know. But the
**unordered pair** does not depend on the assignment:

    f_lo = min(f_A, f_B)        f_hi = max(f_A, f_B)

and it carries the outcome in the only form that matters here. A segment where both
bands burned is a crossing whichever side was windward. A segment where exactly one
burned is a hold or a non-arrival whichever side was windward. So the unordered pair
measures the exclusion bias directly, **requires no side assignment at all**, and is
computable inside the committed accounting script.

> **The check.** For every segment excluded at `SIDE`, `SIDE_2EP` or `TIMING` that
> meets the section 4.0 pixel minimum on both bands, compute `(f_lo, f_hi)` and
> report its distribution against the same pair computed on retained segments:
> **per fire, per barrier type, per side tier, and stratified by `W_cleared`
> tercile**. The summary statistic is the fraction of segments in the
> **one-side-burned configuration**, defined as `f_hi` at or above the section 5.5
> crossed cut and `f_lo` below the section 5.5 held cut, with a cluster bootstrap by
> barrier line.

**Both readings are written in advance, before any value exists.**

- **The excluded set is enriched in the one-side-burned configuration.** The
  censoring runs **against holds**. Holds are then under-represented among labelled
  segments, which pulls the fitted breach probability up at every width and, to the
  extent that the enrichment is flat in width, moves the intercept rather than the
  slope. **This is the conservative direction for the hypothesis** and it is reported
  as such, with the enrichment ratio and its interval stated in the results rather
  than in a limitations paragraph.
- **The excluded set is enriched in the both-bands-burned configuration.** The
  censoring runs against crossings, holds are over-represented, and the fitted breach
  probability is pulled down at every width. This is the **dangerous** direction for
  a design whose hypothesis is that barriers sometimes hold, and in that branch the
  bounding analysis of section 10.5 is run with every excluded segment entered once
  as held and once as crossed, and the width posterior under both extremes is
  reported beside the primary.
- **No detectable difference.** Reported as no detectable difference, with the
  interval and the counts that produced it, and explicitly not as an absence of the
  problem, for the same reason section 12.9 gives.

**Why it is stratified by width tercile rather than pooled, which is A3's addition
and not A6's.** Section 5.1.5 shows that the hold-removing error moves the
**intercept** and leaves the **slope** alone as long as the removal rate is flat in
the covariate. The pooled enrichment ratio cannot distinguish a flat removal from one
that rises with width; only the stratified version can. Since the slope is the
coefficient of interest and the intercept is not, **the stratified reading is the one
that bears on the result** and the pooled one is a summary of it. The reading is
written here too: an enrichment ratio that is itself ordered in `W_cleared` means the
censoring is differential in the covariate, item 2 of section 5.1.5 stops protecting
the slope, and the bounding analysis is run whichever direction the ordering runs.

**What this check cannot do.** It needs the section 4.0 pixel minimum on both bands,
so it says nothing about segments excluded at `CLOUD`, `SCENE` or `FUEL`, which are
exactly the segments with too few usable pixels to compute `(f_lo, f_hi)`. The count
of `SIDE`, `SIDE_2EP` and `TIMING` exclusions that **cannot** enter this check is
reported beside it, so the coverage of the check is itself a number. And it reads
the burned mask on segments that are not labelled, which is permitted because they
are not labelled: section 6.1's provenance firewall governs what may enter a
**covariate** or a **side assignment**, and this is neither. No quantity computed
here enters the design matrix, the side rule or any fit. It enters the accounting
table only, and the audit script whitelists it by name in the same way `n_share` is
whitelisted in section 6.2.
...
10. **Check S0's output** (section 5.1.3): the distribution of the relative front
    irregularity `c` per fire and per barrier type; the realised degrees of freedom
    and the joint `n_det` by `n_epochs` distribution; the pass fraction for tier
    S-A, for tier S-B given an S-A failure, and for the union, at every value of the
    `alpha` grid; the fraction lost to the three-epoch requirement; the plug-in
    wrong-side diagnostic with its declared bias direction; and the rank correlation
    between approach angle and `W_cleared`. Produced before any label exists and
    reported whether or not a label is ever computed.
10b. **Check W0's output** (section 11.3): the R-set counts above and below 6 m of
    `W_cleared` within the road barrier type, and the R-set `kappa_v`. A projection,
    not a gate, reported whichever way it comes out, and the thing that settles
    whether gate W1 or gate M1 binds first (section 17.5).
10c. **The side-invariant exclusion check** (section 12.9.1): the unordered burned
    fraction pair on `SIDE`, `SIDE_2EP` and `TIMING` exclusions against retained
    segments, per fire, per barrier type, per side tier and by `W_cleared` tercile,
    with the count of exclusions the check could not cover.
...
- **The second labelling pass of section 5.8 needs an agent that is not A3** to
  ...
  script. This is condition **C9**, and A6's own ten-unit blind pass is the other
  half of it; neither can run until a candidate set with scenes exists.
- **Condition C1 is A1's and John's**, and it is the one that sets the state of the
  signature. Section 21.1 holds the status of all ten datasets. Nothing in the roads
  tree moves it.
- **Condition C7's reach outside `research/` is WJ-002**, the drafted patch to
  `scripts/check_forbidden.py`, which is a human gate. Until it lands, the standing
  constraint in section 12.4.6 binds this direction's outputs.
- **Condition C10's order reversal is the orchestrator's, A4's and A5's.** What A3
  owes is the prohibition in section 20.5, which is written. What A3 cannot do is
  fix the order for directions it does not own.
- **Condition C2's verification is A6 re-deriving the angle table** of section 5.1.3
  from the declared `alpha` and confirming it matches, and confirming that check S0
  carries `degrees_of_freedom`, `pass_fraction_by_tier` and `pass_fraction_union`.
  All three are key paths in section 22.2 and the table is reproducible from this
  text with a t-distribution and no data.
...
|---|---|---|---|---|
| v0.3 | `research/roads/PREREG_roads_2026-09-16_v0.3.md` | 2026-09-16 | the side cutoff restated as `t_{1 - alpha}(n_det - 3)` at a declared per-tier false-assignment rate of 0.01, with the angle table recomputed and a `c_max` row added, the two tiers budgeted as one two-stage procedure at a union bound of 0.02, and two-epoch segments separated into a declared stratum with `SIDE_2EP` and a pre-registered secondary (5.1.2a, 5.1.2b, 5.1.2c, 5.1.3, C2); the consequence of a wrong side worked out, with the held-versus-crossed arithmetic and the three separate answers on whether it biases the curve (5.1.5, C2); the R-set defined and the attenuation and recoverability instruments, gate M1, gate R0 and section 12.7 pinned to it, with the candidate-set and labelled-set values reported beside (5.1.6, 11.5.1, 11.5.2, 11.5.2b, 11.3, 12.7, C4); the side-invariant unordered-pair exclusion check written with both readings and a width-tercile stratification (12.9.1, C3); `NOEXIST` restated as the width measurement's null result with a declared floor rule, its value at each plausible ground sample distance and a 2 m tripwire, and `NOEXIST` and `WIDTH` added to the clustering readings (3.4.1, 12.9, C5); step A of the attribution test made to record its implied time margin (5.6, C6); gate C1 and projection W0 added to the gate table (11.3); the standing constraint that no width sentence leaves `research/` until WJ-002, and the record-class decision for the now-frozen v0.2 (12.4.6, C7); the P11 non-independence written as a forbidden phrasing with its spellings (20.5, C10); the fit ceiling raised from 48 to 49 by the two-epoch secondary (19.1); check S0 implemented at `research/roads/s0_side_check.py` with synthetic-only tests. Still before any data was touched | A6 signed v0.2 with ten conditions C1 to C10 in `research/eval/signoffs/roads_v0.2.md`, eight of them blocking for the primary result |
...
- **v0.3a**, the segment-level split fingerprint of section 18.2, written
  immediately after the layout script runs and before any label exists. This is
  condition **C8**, and the names v0.3a and v0.3b were pre-announced in v0.2 and are
  kept, now meaning amendments to **this** version rather than to v0.2;
- **v0.3b**, the second-pass disagreement rate of section 5.8, written after the
  ...
  look. This is half of condition **C9**; the other half is A6's own blind pass and
  is recorded in the sign-off tree rather than here.

**Neither amendment can be written in this session and neither is deferred by
choice.** C8 needs the layout script to have run against a real barrier layer, which
needs WJ-017. C9 needs a candidate set with scenes, which needs WJ-009 and WJ-001.
Writing either value now would be writing a number that does not exist, which is the
failure this whole protocol is built to prevent.
...
useful rate, **and condition C2 has made that rate lower than v0.2 expected**, since
the corrected cutoff is 6.97 rather than 2.0 at the bottom of the detection range
and the low `n_det` corner no longer contributes; gate R0 needs the design to be
able to see a 0.05 effect at better than one time in five, **now computed on the
R-set rather than on the candidate set**, which can only lower it; gate W1 needs 25
road segments on each side of 6 m of `W_cleared`; and gate M1 now reads the lower end
of an interval, which by section 11.3's arithmetic wants a point `kappa_v` of at
least about 0.77, and A6 is right that 0.77 is a floor rather than the figure,
because that derivation treats `Var(v_true)` as known when it is estimated on a
support two of this design's own filters truncate.

**On which gate fails first, A3 declines to predict and builds the instrument
instead.** A6's record moves its expectation to gate W1 failing ahead of gate M1.
A3's reading of the mechanism differs in one respect that is set out in section 17.5,
and rather than trade predictions, this version adds **check W0** in section 11.3: a
pre-labelling projection, on the R-set, of the two quantities W1 and M1 read. W0's
6 m counts are an upper bound on W1's, so a W0 failure is a W1 failure; W0's
`kappa_v` is M1's own input on M1's own geometry. **The ordering therefore becomes a
reported number available before any labelling effort is spent**, and this document
asserts nothing about it. That is the same move this design makes everywhere else,
and it is the right answer to a disagreement between two agents neither of whom has
seen a single Korean road segment.
...
### 17.3 Where A3 disagreed with the refusal of v0.1b, and how A6 ruled

Per `SIGNOFF.md` section 5 there is no negotiation between agents, and A3 did not
open one. **All three were upheld by A6 in `research/eval/signoffs/roads_v0.2.md`
section 5.** They are kept here with A6's ruling attached, because how a design
reached its shape is part of its record, and because the first of them is now part
of the signed design and is not reopenable without a version bump.
...
**A6's ruling on item 1.** Conceded without reservation. A6 records that its own
R1 remedy list named two mechanisms both derived from the burned mask, that assigning
a side from either would let the label define its own geometry and manufacture `HELD`
on exactly the segments the hypothesis is about, and that this was reviewer error of
the kind A6 exists to catch in others. **A3's extension of the section 6.1 provenance
firewall to the side rule (5.1.2) and to the smoke reference region (5.2.1) is
ratified as part of the signed design and is not reopenable without a version bump.**
A6 takes a second consequence, that offering candidate remedies inside a refusal is
itself a hazard, and records it as a limitation of its own round-2 record.

**A6's ruling on item 2.** Conceded on the mechanics: A3's correction is right and
A6's "majority of segments" was wrong. A6 adds a residual that is not a defence of
S4 and that A3 accepts in full: if most `CROSSED` labels are reached on geometry
alone, the attribution test performs no attribution on most of the positive class,
and the guard rests on 500 m being far enough against a timing uncertainty section
12.11 already reports can exceed two hours. That residual is **condition C6** and is
answered in section 5.6, where step A now records the time margin it implicitly
asserts. No label changes.
...
**A6's ruling on item 3.** Upheld, and A6 adds two things that make the cost larger
and neither of which changes the ruling: the derivation treats `Var(v_true)` as known
when it is estimated on a truncated support, so **0.77 is a floor on the point
requirement and not the requirement**; and `kappa_v` is a posterior quantity from the
joint fit rather than a plug-in, so the number is indicative of direction and size,
which is all it needs to be. Both are carried into section 11.5.1.

### 17.4 What this version does not fix

Four things, named so that a signature is not read as covering them.
...
- **The two datasets that decide whether the direction has a question are still
  ...
  decides. Under P12 that caps this at `signed with conditions`, which is condition
  C1 and is unchanged since v0.2. **Nothing in v0.3 moves it**, and v0.3 should not
  be read as moving the direction closer to a fit. It moves the design closer to
  being correct when a fit becomes possible, which is a different axis.
- **The second-pass disagreement rate of section 5.8 does not exist yet.** The
  procedure is frozen; the number arrives at v0.3b. Condition C9.
- **The segment-level split fingerprint does not exist yet**, because the layout
  ...
  the segment-level one arrives at v0.3a. Condition C8.
- **Condition C2's two-epoch repair is a separation and not a correction.** Section
  5.1.2c removes two-epoch segments from the primary because the bias in `sigma_r`
  cannot be quantified by any argument in this document. If the Korean record turns
  out to be mostly two-epoch segments, the primary loses most of its sample and the
  secondary carries a bias of unknown size. Check S0 measures which of those is the
  case, before labelling, and the document commits to the primary either way.

### 17.5 Where A3 reads A6's conditions differently

Two places. **Both conditions are satisfied as written**, in section 11.3 and section
5.1.4 respectively, and neither reading changes anything this version does. They are
recorded because A6's record asks to be told when its reasoning is read differently,
and because one of them is checkable and is now checked.

**1. The two truncations do not both reach gate W1, and A3 therefore does not adopt
the prediction that W1 fails before M1.** A6's section 4 argues that the roads width
support is squeezed from below by the existence check and from above by the side
rule, "both through the covariate's own machinery", and concludes that **W1** is now
expected to fail before **M1**. A3 agrees with every step of the mechanism and with
the conclusion's direction of concern, and reads the last step differently:

- the **lower** truncation, `NOEXIST`, acts on `W_cleared` directly (section 3.4.1),
  and W1 counts in `W_cleared` by A6's own ruling. So this one reaches W1;
- the **upper** truncation acts on `W_eff`, through `sin(theta)`, and `theta` is a
  property of the front's bearing against the barrier's bearing rather than of the
  road's design class. It reaches `Var(v_true)`, hence `kappa_v`, hence **M1**. It
  reaches W1 only through whatever association exists between `theta` and
  `W_cleared`;
- so the squeezed-from-both-ends picture is a statement about **`kappa_v` and gate
  M1**, and gate W1 is squeezed from below only.

**A3 does not conclude from this that M1 fails first.** The point is the opposite:
**neither ordering is derivable**, because the `NOEXIST` rate depends on a ground
sample distance nobody has, and the `theta`-`W_cleared` association is an empirical
fact about Korean forest roads that nobody has measured. What A3 does instead is make
both quantities pre-labelling reported numbers: **check W0** in section 11.3 projects
W1's counts on the R-set and reports M1's own input beside them, and **check S0**
reports the `theta` to `W_cleared` rank correlation at
`checks.S0.theta_vs_wcleared_rho`, which is exactly the quantity that decides whether
the upper truncation reaches W1 at all. A6's prediction and A3's qualification are
both scoreable against those two numbers, and both were written before either exists.

**2. Condition C4's placement relative to gate R0 is right, and its scope needs one
sentence A6 does not give it.** C4 requires the instruments to run on the retained
geometry and gate R0 to read the retained value. Adopted in full. The sentence that
is missing is that **the retained set is not the fitted set**: the R-set of section
5.1.6 is everything determinable before the scenes are read, and section 5.7 removes
more segments afterwards, at steps 3, 4, 6, 7 and 8, in a way section 12.9 already
expects to be outcome dependent. So an R-set instrument still overstates the power of
the fit that runs, by less than a candidate-set instrument and by more than nothing.
The gates have to fire before labelling or gate R0 has no purpose, so this cannot be
fixed by moving them. Section 5.1.6 therefore adds a third, non-gating value computed
on the labelled set and reported beside the other two, which turns the residual
overstatement into a measured quantity. That is an addition to C4 rather than a
qualification of it.
...
is 49 fits on real labels**, raised from v0.2's 48 by exactly one, the two-epoch
secondary that condition C2 creates (section 5.1.2c). The raise is named here rather
than absorbed, because a ceiling that moves quietly is not a ceiling. A fit that is not in this table is a version bump, is
...
| S-A tier only (5.1.2b), now unconditional rather than conditional on a materiality judgement | 1 |
| two-epoch secondary, admitting `SIDE_2EP` segments (5.1.2c) | 1 |
...
| side-test `alpha` and `d_bypass_min` grids (12.8) | 4 |
...
**A6 accepted the declaration and made item 3 a condition rather than a
recommendation, which is C10.** A6's reasoning is worth carrying: the same defect
occurring twice would be a pattern rather than an accident. A6 also prices the defect
in a way A3 did not, and the pricing is recorded here because it bounds the damage.
The contamination ran in the **cheaper** direction: the author read the reviewer's
list, and the reverse would have been much worse, because a reviewer starting from
the author's framing inherits the author's blind spots and the review becomes a
ratification. A6's round-2 pass over the twelve roads items was run independently,
before A3 wrote anything, and it is that pass which produced B3 and B4 at CRITICAL
unresolved. **So the round did receive one genuinely independent audit; what it did
not receive is a second one.** A6 further records that the single differing verdict,
B10, carries little information, because it differs by A3 supplying a mitigation
rather than by disputing a reading, which is what contamination predicts, and that it
should not be counted as a disagreement that survived independent formation.

**What C10 requires of this document, and section 20.5 is where it lands.**
...
than by A3 disagreeing with A6's reading. **A6 rules that it should not be counted as
a disagreement that survived independent formation, and A3 accepts that**, so this
section records one marked difference and claims nothing from it. Two further points
where A3 disagrees with A6's **reasoning** rather than with a verdict are in section
17.3, and two on A6's v0.2 conditions are in section 17.5, because none of them are
checklist items.

### 20.5 What section 20 may not be summarised as, which is condition C10

**Part A and Part B of this section are not independent evidence and must never be
reported as though they were.** A3's verdicts were written after reading A6's, the
defect is declared in section 20.1, and A6 has made the consequence a condition.

> **Forbidden, in any report, poster, abstract, figure caption, judge Q&A card or
> paper sentence, for this direction, permanently, and not only while conditions are
> open:** any statement that the leakage audit was independently corroborated,
> cross-checked, confirmed by two audits, or that the author's audit and the
> reviewer's audit agree. The spellings this covers include, and are not limited to,
> "two independent leakage audits", "both audits agree", "independently
> corroborated", "cross-validated against A6's audit", "the author and the reviewer
> reached the same verdicts", "독립적으로 교차 검증" applied to this audit.

**What may be said**, so the prohibition does not push a later author into silence
where an honest sentence exists: that A6 ran an independent leakage pass in round 2
before the author wrote anything; that the author's verdicts were written afterwards,
with the order defect declared; and that the author's verdict list is a commitment a
later reader can hold this direction to, which is what it is worth and is not nothing.
A verdict of "mitigated" on B3 written before any fit exists is a commitment of that
kind.

**Why this is written as a phrasing rule and not only as a note.** The claim this
forbids is exactly the claim a compressed summary produces by default: two lists of
verdicts that mostly agree are what "two audits agree" describes, and the fact that
makes it false, the order they were written in, is the fact a summary drops first.
This is the same failure mode section 12.4.6 exists for, in a different vocabulary,
and it is recorded in the same shape.

**Not currently machine-checked, and that is stated rather than hidden.**
`research/roads/check_vocabulary.py` carries RV-001 to RV-003, which are about the
estimand, and none of them fires on the spellings above. A fourth rule is not added
in this version, because A3 has no corpus of legitimate neighbours for it yet, and
`FORBIDDEN_CLAIMS.md`'s own standard is that a rule firing on legitimate hedged
wording is worse than no rule because agents learn to ignore the checker. What
exists instead is the list of spellings above, which is what a later author or
reviewer greps for. If C10's prohibition survives to the point where a summary of
this direction is actually being written, the rule is drafted then, with its catches
and its spares, by the agent that needs it.
...
**A6 states it in a sharper form and A3 adopts the sharper form.** The two verified
files supply containment time and a cross-check on it. Neither supplies a covariate,
a label, a barrier, a scene, a detection, a terrain value or a weather value. So the
honest one-line statement of the data position is not "eight of ten pending", it is
**"nothing that can produce a number has arrived"**. That includes both of the two
whose absence would end the direction outright: the forest road layer behind WJ-017
and the pre-fire orthoimagery behind WJ-009.

**This is condition C1**, it is unchanged since v0.2, and it is not A3's to move. It
is written as gate C1 in section 11.3 so that it carries a boolean in the artifact
like every other precondition. Nothing in v0.3 changes any status in this table, and
v0.3 should not be read as moving this direction closer to a fit.
...
### 22.1 The artifact
    ...
    research/roads/results/roads_prereg_v0_3_results.json
...
| `measurement.kappa_v.rset.mean`, `.ci90_low`, `.ci90_high` | the attenuation on the model's scale, **on the R-set of section 5.1.6**. **M1 reads `.ci90_low`**. Condition C4 |
| `measurement.kappa_v.candidate.mean`, `.ci90_low`, `.ci90_high` | the same on the full candidate set, reported for comparison, gating nothing. Condition C4 |
| `measurement.kappa_v.labelled.mean`, `.ci90_low`, `.ci90_high` | the same on the labelled set, computed after labelling, gating nothing, the measure of how much the pre-labelling instruments overstated (5.1.6) |
| `measurement.var_v_true.rset`, `.candidate` | mechanism 1 of section 5.1.4, as two numbers |
| `measurement.mean_sigma_v_sq.rset`, `.candidate` | mechanism 2 of section 5.1.4, as two numbers |
| `measurement.weff_ceiling_by_fire[].fire`, `.value` | mechanism 3 of section 5.1.4, the realised `W_eff` ceiling per fire |
| `measurement.kappa_v_per_segment.deciles[]` | the per-segment distribution, on the R-set |
...
| `recoverability.geometry` | which set instruments 2 and 2b ran on. Condition C4 requires this to read `rset` for the value gate R0 uses |
| `recoverability.detection_power.rset.value`, `.threshold`, `.at_effect` | instrument 2 on the R-set, at 0.05 and at 0.10. **Gate R0 reads this one** |
| `recoverability.detection_power.candidate.value`, `.threshold`, `.at_effect` | the same on the candidate set, reported beside it so the overstatement is visible |
| `recoverability.detection_power.labelled.value` | recomputed after labelling, gating nothing (5.1.6) |
| `recoverability.exclusion_power.rset.value`, `.threshold`, `.at_effect` | instrument 2b on the R-set, gate M2 |
| `recoverability.exclusion_power.candidate.value` | the same on the candidate set |
...
`gates.C1.passed` / `.value`, `gates.E1.passed` / `.value`, and the same for `S1`,
`R0`, `L1`, `G1`, `G2`, `G3`, `W1`, `N1`, `H1`, `M1`, `M2`, `P1`. Fourteen booleans
and fourteen quantities, one more than v0.2 because condition C1 is now a gate with a
boolean rather than a sentence. A gate's outcome is a result and it is reported
whether it passed or failed.

Three further gate-adjacent key paths, none of which is a boolean:

`gates.E1.route` in {`edition`, `existence`, `both`}, which condition C5(c) requires,
so a pass obtained by the existence check alone is reported under that name;
`gates.R0.geometry`, which must read `rset`;
`checks.W0.n_wcleared_ge_6m`, `.n_wcleared_lt_6m`, `.kappa_v_rset`, the projection of
section 11.3 that is not a gate and fires no branch.
...
| `labels.counts.by_fire[].by_reason.{SCENE,TIMING,SIDE,SIDE_2EP,CLOUD,MID,DETACH,FLANK,FLANK_AMBIG}` | indeterminate by code. `SIDE_2EP` is new at v0.3 (5.1.2c) |
...
| `labels.crossed_reachability.by_geometry_thin_margin` | condition C6: the count, per fire, of geometric passes whose implied flanking delay `d_bypass / r_flank` falls below the combined standard error of `t_w` and the acquisition time |
| `labels.crossed_reachability.geometry_margin_hours.deciles[]` | the margin's distribution, so the count above is legible |
| `labels.side_wrong_side_bound` | `alpha_union / q` from section 5.1.5, the bound on the depression of the held-versus-crossed ratio, computed from the realised union pass fraction |

**The pre-labelling checks and the sensitivity register**

`checks.S0.c_distribution.deciles[]`,
`checks.S0.degrees_of_freedom.deciles[]` and `.by_fire[]`,
`checks.S0.n_det_by_n_epochs{}`,
`checks.S0.pass_fraction_by_tier{}` with keys `S_A` and `S_B_given_S_A_fail`,
`checks.S0.pass_fraction_union{}`, all three keyed by the value of `alpha` in
{0.005, 0.01, 0.025} and all three reported per fire,
`checks.S0.lost_to_three_epoch_requirement{}` keyed by the C4 epoch grid value,
`checks.S0.wrong_side_plugin.value` and `.bias_direction`, the diagnostic of section
5.1.5 with its declared understatement,
`checks.S0.theta_vs_wcleared_rho` and `.ci90_low`, `.ci90_high`, the rank correlation
that decides whether the upper truncation reaches gate W1 (sections 5.1.5 item 2 and
17.5),
`checks.S0.t_crit_by_segment.deciles[]`,
...
for the section 12.9 clustering check, now over the codes `NOBURN`, `NOTMAIN`,
`NODETECT`, `TIMING`, `SIDE`, `SIDE_2EP`, `NOEXIST` and `WIDTH`,

and, for the side-invariant exclusion check of section 12.9.1, which is condition C3:

`selection.side_invariant.excluded.one_side_burned_fraction`, `.ci90_low`,
`.ci90_high`,
`selection.side_invariant.retained.one_side_burned_fraction`, `.ci90_low`,
`.ci90_high`,
`selection.side_invariant.enrichment_ratio`, `.ci90_low`, `.ci90_high`, `.reading`,
`selection.side_invariant.by_wcleared_tercile[].tercile`, `.enrichment_ratio`,
`.ci90_low`, `.ci90_high`, which is the stratified reading that bears on the slope,
`selection.side_invariant.by_fire[]`, `.by_barrier_type[]`, `.by_side_tier[]`,
`selection.side_invariant.f_lo_deciles[]`, `.f_hi_deciles[]` for both sets,
`selection.side_invariant.n_excluded_not_coverable`, the count of `SIDE`, `SIDE_2EP`
and `TIMING` exclusions that fail the section 4.0 pixel minimum on a band and
therefore cannot enter the check at all,

and, for condition C5(a),

`measurement.existence_floor.gsd_m`, `.w_floor_m`, `.vintage`, per segment and
summarised per fire, with `.tripwire_fired` true when the delivered ground sample
distance is 2.0 m or coarser.
...
[
  {
    ...
    "source_file": "research/roads/results/roads_prereg_v0_3_results.json",
...
### 22.4a The code committed with this version

Three files, all inside `research/roads/`, all of which run today and none of which
reads anything from disk.

| file | what it is | state |
|---|---|---|
| `research/roads/check_vocabulary.py` | the RV-001 to RV-003 vocabulary ratchet of section 12.4.6 | committed at v0.2, unchanged |
| `research/roads/s0_side_check.py` | **check S0** (section 5.1.3): the plane fit, the side statistic, `t_crit(n_det - 3, alpha)`, the two-tier rule with the two-epoch separation, the pass fractions per tier and for the union, and the section 5.1.5 plug-in diagnostic. `--tables` prints the section 5.1.3 tables from the t distribution and no data | **new at v0.3** |
| `research/roads/recoverability.py` | **instruments 2 and 2b** (sections 11.5.2, 11.5.2b): the geometry object that carries its own name per condition C4, the contrast algebra, and the two power arms behind a named backend | **new at v0.3** |
| `research/roads/tests/test_s0_side_check.py` | 15 tests, including the Monte Carlo demonstration that the t cutoff is calibrated at two residual degrees of freedom and a fixed 2.0 is not, and a structural test that check S0 opens no file | **new at v0.3** |
| `research/roads/tests/test_recoverability.py` | 13 tests, including the demonstration that the candidate set overstates the R-set, which is the defect condition C4 closes | **new at v0.3** |

**Every fixture in those tests is synthetic, generated in-process from a declared
seed, and is not Korean data.** No number produced by any of them is a measurement
and none may be quoted anywhere. The Korean detection record is behind WJ-001 and
the orthoimagery behind WJ-009; neither has been downloaded and neither file
attempts to. The structural test in `test_s0_side_check.py` is the check on that
claim: it parses check S0 and asserts that it imports no raster or vector reader
and calls nothing that opens a path.

**Why these two and not others.** A6 named the buildable-unsigned work in order:
check S0, then the recoverability harness, both of which run on simulated outcomes
and need no scene and no label. Condition C2 had to land before check S0 because it
changes what the test is, and it has. What the code buys beyond the document is that
the section 5.1.3 tables are now **generated** rather than typed, so A6's
verification action for condition C2, re-deriving the angle table from the declared
`alpha`, is a command rather than a transcription, and the null calibration that
C2 asserts is re-run by a test rather than asserted by a paragraph.
