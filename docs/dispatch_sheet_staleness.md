...
`data/processed/dispatch_sheet_staleness/staleness_20260912T092708Z.json`,
...
커밋 시트는 있을 수 없습니다. 이 0은 위의 44를 읽을 수 있게 해 주는 **대조군**입니다:
0이 아니었다면 커밋된 트리가 기록 하나와 수리 하나가 아니라 **살아 있는 문장 둘**을
들고 있다는 뜻이 됩니다. 나중에 새 run을 커밋하는 랩은 이 칸을 움직이게 되고,
`tests/test_dispatch_sheet_staleness.py::test_no_committed_sheet_carries_the_current_sentence`
가 그 랩에게 이 절을 같이 고치라고 말합니다.

## 4. 한계 — PDF는 한 장도 열지 않았습니다

이 측정의 가장 중요한 한 줄입니다. 이 샌드박스에는 **PDF 텍스트 추출기가 없습니다** —
`pypdf` 도, `pdfminer` 도, `pdftotext` 도 없습니다. 그래서 **PDF 바이트는 읽히지
않았고**, 각 PDF는 그것이 렌더링되어 나온 형제 HTML로 등급이 매겨졌습니다.

- `dss_stale_committed_pdfs` 는 「**옛 문장을 가진 HTML에서 렌더링되었다**」는 뜻이지
  「열어 보니 그 문장이 있었다」가 아닙니다.
- 이 추론이 깨지는 경우는 실재합니다: HTML이 쓰인 뒤 PDF가 다른 입력으로 다시
  만들어졌거나, 손으로 갈아 끼워졌거나, 렌더링이 중간에 실패해 옛 PDF가 남았을 때.
  저장소의 어떤 것도 그런 일이 없었음을 증명하지 않습니다.
- 추출기가 있는 기계는 이 추론을 **읽기**로 승격시킬 수 있습니다. 페이지 수를 렌더러
  있는 기계가 재야 하는 `WFG-116`과 같은 모양의 부채입니다.
...
5. **이 목록은 손으로 관리되지 않습니다.** 열거는 `outputs/dispatch/README.md` 에 있고,
   게이트가 트리에서 **다시 유도해** 그 문서와 대조합니다. 오늘의 열거를 테스트에
   박아 넣지 않은 것은 일부러입니다 — 그런 게이트는 오늘의 사각지대를 그대로
   물려받습니다(`WFG-266` done-when (d), `WFG-271`의 다시 쓰인 done-when).
