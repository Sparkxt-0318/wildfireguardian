...
| [`ordering_boundary.md`](ordering_boundary.md) | **⚠ PHASE 24 (2026-08-11) — the W axis of PHASE 23 filled in from 2 points to 12 (60…600), 2,160 cells. It does NOT reopen PHASE 23: that run's W=75 and W=240 values were re-derived cell by cell here, 3,744 values, **0 differences**. What is new is the boundary of the NEGATIVE result. Win rate leaves 0 at **W = 120** (1.6× the committed 75), but never rises above **12.2 %** even at W = 600, the loss rate stays 68–83 % across the whole axis, and the mean margin is negative at **all twelve** windows. **No single threshold exists** — not in W (min winning 120, max non-winning 600: the ranges fully overlap) and not in the distinct-deadline count (non-monotone; 1,580 non-winning cells sit at or above the lowest winning value). 87 % of the wins need a 60-min dispatch delay, and 1 team never wins anywhere. §8: `W = 75` is marked `ASSUMED` at `config/default.yaml:365` with **no measured basis anywhere in the tree**, so the distance to the boundary is a statement about an assumed window. ⚠ Read §1 before quoting anything: "a valid condition exists" and "the current condition is that condition" are different claims, and the second is still false.** |
