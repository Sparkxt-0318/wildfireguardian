...
**The alternative-venue survey, carried forward twice, was done this run — and its answer is that switching venue does not avoid the money question.** IGARSS 2027, the flagship conference of the IEEE Geoscience and Remote Sensing Society, is in **Reykjavík, Iceland, 2027-07-11 → 16**, theme 「Global Vision for a Changing Planet」, call for papers open (<https://2027.ieeeigarss.org/>, read 2026-09-10). ⚠ Its site did not state deadlines or fees on the page this run opened, so **no figure is recorded here**. But the structural point does not need one: **IEEE conference publication requires at least one author to register and attend**, so a conference route substitutes a registration-and-travel cost for an APC rather than removing a cost. There is therefore **no free IEEE route**, and §4's item 1 is not avoidable by venue choice — it is a decision the author must make either way. That is the honest result of the survey and it is better to have it now than in 2027-01. ⚠ A second point in IGARSS's favour regardless of cost: its **date (July 2027) falls after the ISEF 2027 window (2027-01 → 05)**, which keeps the prior-publication question in §4 item 4 simpler than a spring submission would.
...
| G11 | **Related work does not situate the routing arm against a same-architecture comparator.** Bokade et al. (2026-09-09, 10.5281/zenodo.22668357) couple a U-Net flammability prior to a CA physics engine and route with OpenRouteService plus a D* Lite fallback that routes away from *actively predicted* fires — the same pipeline shape, published in the same month. A reviewer who finds it will read an unsituated architecture claim as the contribution. The fix states the delta as the **output object and its measured limits** (they report no routing evaluation at all), plus why time-expansion rather than incremental replanning | WFG-239 (research lap, 2026-09-10) | before freeze |
...
- **2026-09-10** — the **alternative-venue survey finally ran** (§1): IGARSS 2027 recorded as a concrete IEEE GRSS candidate with its dates and theme, and the survey's real finding written down — an IEEE conference swaps an APC for author registration and travel, so **no IEEE route is free** and §4 item 1 is unavoidable by venue choice. One gap added (**G11**, the routing arm unsituated against a same-month same-architecture comparator) with the row that closes it. §2 gains nothing this run: no source opened reported a transfer experiment. **Nothing was submitted anywhere.**
