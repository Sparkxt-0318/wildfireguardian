...
*Research routine, sandbox. Scan window: new since 2026-09-05. Sources below were opened at the URLs given; nothing above is deleted.*

**1. The condensed time-expanded network, and why it names the project's own design choice.** Chawla & Sheridan (2026-04-30, arXiv:2605.00277, "Brief announcement: A special case of maximum flow over time with network changes", <https://arxiv.org/abs/2605.00277>) [opened] consider a network with uniform edge lengths whose **edge capacities change only at μ discrete _critical times_**, and show how to build a **condensed time-expanded network (cTEN)** whose ordinary max flow equals the max flow over time on the original. For a graph with n nodes and m edges the cTEN has O(n²μ) nodes and O(μmn) edges, giving O(μ²n³m) with Orlin's algorithm; the authors say plainly that they are interested in graphs with *many* changes, because that is where μ dominates. **Why this matters here even though the project solves a shortest path and not a flow.** §1.3 above records that WildfireGuardian's router expands time into fixed 10-minute bins, and `docs/routing_limitations.md` §1 records the artefact that follows (`fa_exceeds_budget` is a quantisation misnomer). Chawla & Sheridan's framing says what the *right* discretisation parameter is: **not the clock, but the number of times the network actually changes.** This project's canonical hazard field is a stack of slices, so its hazard is piecewise-constant with a handful of breakpoints, and a bin grid finer than those breakpoints buys nothing and costs the artefact. ⚠ The honest boundary, and the first reading of this that a lap should not act on: **the bins are not only the hazard's discretisation, they are also the Dijkstra label's arrival-time bookkeeping.** Removing them is not available. What *is* available, and is exactly candidate **B1** in §7.2 above, is evaluating the *gate* at the exact interpolated arrival time against the piecewise-constant field, which is an exact operation precisely because the field has few breakpoints. Chawla & Sheridan is the citation that makes that a principled statement rather than a tweak.

**2. Supported evacuation of the immobile is a named problem with a formulation.** Moradi, Sauré & Patrick (2026-08-05, arXiv:2608.05413, <https://arxiv.org/abs/2608.05413>) [opened] define **supported evacuation** — evacuating people who cannot self-evacuate, their examples being hospital patients and long-term-care residents — as a two-stage stochastic program that jointly optimises **facility location, fleet sizing and vehicle routing under hard time windows**, solved by logic-based Benders decomposition with combinatorial Benders cuts, and validated against a community wildfire drill in Roxborough Park, Colorado. The abstract reports improvement in shelter placement, vehicle utilisation and evacuation efficiency against alternative policies and states no headline number. **What it changes here:** this is the closest published framing of WildfireGuardian's rescue-dispatch layer, and it names the layer's problem class. It also marks the boundary of what this project claims: the project *classifies* who needs support and asks whether a corridor survives long enough to reach them; it does not solve fleet sizing or facility location jointly, and it is single-stage and deterministic given a field. `docs/dispatch_ordering.md`'s negative result on ordering (§7.1 insight 9) is consistent with that — the ordering question is the easy end of a problem whose hard end is the two-stage one. It belongs in the related-work table (WFG-026) beside Solomon (1987) and Jagtenberg et al. (2017).

**3. Shelter location under uncertainty, for the refuge question.** Adaptive robust optimisation for evacuation planning — shelter location, evacuation routing and fleet decisions made *before* the number and spatial distribution of evacuees is known (arXiv:2608.04225, 2026-08-04, <https://arxiv.org/abs/2608.04225>) [abstract, listing page] — is the same family as §5 above and is the formulation G6's refuge-provenance question runs into if it is ever asked as a *placement* question rather than a *provenance* one. Recorded, not adopted: this project does not place refuges and must not start.

**4. Two systems the routing note should know exist, and one it should not chase.** `KOREAN_OPERATIONAL_SYSTEMS.md`, new this run, records NIFoS's AI 산불확산예측시스템 and 경기도's G-DAPS. Both compute spread; neither routes a pedestrian. Separately, BEACON (arXiv:2609.03301, 2026-09-03, <https://arxiv.org/abs/2609.03301>) [opened] is a multilingual evacuation-guidance app whose routing is a 「polygon-avoidant routing pipeline」 around fire perimeters — i.e. avoid where the fire **is**, which is the fire-blind-plus-perimeter opponent this repository already argues about in `docs/fair_opponent_line.md`. It reports no evaluation. It is a useful citation for 「what deployed guidance apps actually do」 and nothing more.

### What this means for WildfireGuardian — revision of §7.1, 2026-09-06

The ten insights stand. Three are amended in place:

- **Insight 3 is upgraded from a framing to a citable design principle.** 「Bin quantisation is a time-expansion resolution choice」 now has a paper that says the resolution parameter should be the number of **critical times** at which the network changes (Chawla & Sheridan 2026), not the clock. The project's hazard changes at a handful of slice boundaries; its bins are finer than that. **B1 is the experiment this justifies**, and this run files the cheap screen that decides whether B1 is worth a lap at all (**WFG-143**).
- **Insight 9 gains its problem class.** The dispatch layer's honest position is that it answers a feasibility question inside a problem whose full statement is Moradi et al.'s two-stage supported-evacuation program. Say that in related work; it is a stronger and more accurate sentence than the current one, and it costs nothing.
- **Insight 10 gains a neighbour.** Refuge *placement* under uncertainty (arXiv:2608.04225) is a live formulation. The project's claim remains the b→c contrast on a fixed refuge set, and placement stays out of scope before the finals.

### Backlog candidates from this update

| id | question | data | when | rules |
|---|---|---|---|---|
| **B7** | Of the 458 canonical Yeongdeok origins, how many have a routed arrival time within one bin width of a hazard slice boundary? That set is a strict **upper bound** on the origins whose bucket can change if the gate is evaluated at the exact interpolated arrival time (B1). If it is empty, B1 is dead for the cost of one read; if it is not, B1 has a pre-registered ceiling before a lap spends on it | the committed routing artifact alone — no re-routing, no refit, no artifact edited | **before-freeze**; filed this run as **WFG-143** | yes — reads a committed artifact, writes a new one under a new filename |

## Update 2026-09-08

*Research routine, sandbox. Scan window: new since 2026-09-06, through OpenAlex (new channel this run) as well as arXiv. Sources below were opened at the URLs given; nothing above is deleted.*

**1. The route-existence question has a published method, for static hazards, and naming it makes this project's claim smaller and much harder to attack.** Opanasopit & Louis (2026-08-06, *Frontiers in Built Environment*, 10.3389/fbuil.2026.1856100, <https://doi.org/10.3389/fbuil.2026.1856100>) [opened] present a GIS framework that identifies **isolated communities** after a disaster by taking the surviving road network as an undirected NetworkX graph and running **connected-component analysis** to find components that have lost ground connectivity to the wider regional network. Population-level accessibility is then a Census-Block **road-length ratio** — surviving connected road length ÷ pre-disaster road length, multiplied by block population — computed separately per critical-facility type. Applied to **Clatsop County, Oregon** under earthquake, tsunami, flood, landslide and windstorm scenarios, the combined all-hazards case yields **70 isolated communities**, with the population retaining access to critical facilities falling from **38,100 to 11,900**; flooding produced the most fragments while tsunami produced the largest accessibility loss, and one airstrip was the only road-accessible airport under tsunami and flood, which the authors name as single-access-route vulnerability.

**What this changes here, and it is a framing correction rather than a method import.** §0 of this note describes what the project's router *is*; nothing in §1–§6 situates the project's **route-existence** result — the claim that whether a corridor exists at all dominates how it is optimised — against any published literature, because none had been found. This is that literature, and it is close: same graph object, same connected-component question, same accessibility-of-population output. The difference is exact and is worth writing in one sentence, because it is the whole contribution:

> **Opanasopit & Louis delete edges under a static post-event hazard footprint and ask who is disconnected. This project deletes an edge's *usability as a function of the time a walker would traverse it* under a moving, forecast hazard field, and asks who can still get out before the field closes the corridor. Static severance against time-dependent severance under a forecast.**

⚠ **Boundaries, and they belong in the sentence itself.** Different hazard family (earthquake/tsunami/flood, not fire), different country, different unit (Census Blocks and a vehicle road network, not households on a walk graph), and their accessibility proxy is a *road-length ratio* rather than a routed arrival time — so it validates the **method class** this project sits in and **never** any fire number here. Stating the delta is also strictly weaker in scope than the unsituated version, and that is the point: an invention claim dies to one citation, a delta claim does not. Filed as **WFG-198** — a related-work line and a Q&A card for 「기존 접근성 분석과 뭐가 다릅니까」, no code and no number.

**2. Two evacuation-modelling items surfaced that this run could not open, recorded as leads only.** 「Addressing congestion in time-expanded networks: a lifeboat allocation model for maritime evacuations」 (*Top*, 2026-07-16, 10.1007/s11750-026-00724-7) and 「Evac-cast: an interpretable machine-learning framework for evacuation prediction」 (*Safety Science*, 2026-08-21, 10.1016/j.ssci.2026.107391). **Both are paywalled to this sandbox — Springer redirected to `idp.springer.com`, Elsevier returned 403 — and neither has been read, so nothing about their content is written here.** The first is flagged because its title touches **P-001**'s parking (capacity and congestion in a time-expanded network): if a later run opens it and it demonstrates a regime where congestion binds at small population scales, that is precisely the evidence P-001 says would revive it. It is not evidence today.

**3. Korea's staged-evacuation doctrine puts a clock on the routing problem.** NIFoS's 2026-02-12 strategy, reported in 헤럴드경제 (<https://biz.heraldcorp.com/article/10675702>) [opened], sets a 「준비(Ready)-실행 대기(Set)-즉시 실행(Go)」 ladder in which 고령자 등 안전 취약계층 begin preemptive evacuation at **8 h before fire-line arrival** and are to have completed the move by **5 h before**. That is recorded in full in `KOREAN_OPERATIONAL_SYSTEMS.md` §Update 2026-09-08 and matters to this note for one reason: it means the routing layer's operative question is not 「what is the shortest safe path」 but 「is this household's path still usable at the moment the doctrine says the decision is made」, which is the time-dependent-severance question of §1 above stated as a policy deadline. It changes no method here and no number anywhere.

### What this means for WildfireGuardian — revision of §7.1, 2026-09-08

The ten insights stand. Two are amended in place:

- **A new insight is added, and it is the one this note was missing.** *Route existence is an instance of connected-component accessibility analysis, and the project's contribution is the time-dependence, not the question.* Situate it against Opanasopit & Louis (2026) in related work and at the booth; the delta sentence in §1 above is the deliverable, and it is stronger than the unsituated claim it replaces.
- **Insight 10's neighbour list grows by a boundary, not a method.** Refuge *placement* stays out of scope (2026-09-06), and accessibility *measurement* is now situated (this run). Neither is adopted; both are cited.
