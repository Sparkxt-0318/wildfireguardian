...
⚠⚠ **Update, critic #73, 2026-09-12T0822Z. The proxy is now the ONLY length measurement that exists, because the real one was nulled today, correctly.** Measured at `4bf34ab`: `paper/STATE.json` `built_pages` is **`null`**, and `built_pages_note` records why — the WFG-266 dev lap repointed two figures to `F3b_regions.png` and `F8b_routing_map.png`, which changes the inputs digest, and it **nulled the anchor rather than re-anchoring it**, because this sandbox has no renderer and hand-writing a number would keep a page count no run produced. That was the right call and this update is not a complaint about it.

**What it changes for you.** Until today the entry could say 「the last measurement that did run put the document at 23 pages」. There is now **no** page measurement at this head at all. Your 25-page rule (NH-028) is verified by **nothing**, the sprint ends **2026-09-15**, and the one thing that would fix it is a minute on a machine with LibreOffice Writer: `.auto/venv/bin/python paper/measure_pages.py`. The loop cannot do it (**WFG-116**, `todo`, P1, held behind R3 by CHARTER §14b).

This adds no new question and changes no option. It is here so that your answer to NH-037 is weighed against 「no page number exists」 rather than 「the page number is 23 and the proxy stops us two pages early」.

⚠⚠ **Update, critic #74, 2026-09-12T1125Z. Both halves of the update above have moved in the same window, and one of them moved to zero.** Measured at `092c907` in this lap's own process, not read from the paper lap's report.

- **The page count is back and it is real.** `paper/STATE.json` `built_pages` is **23** with `built_pages_inputs` `7ed51c97137c9b2c`, and `.auto/venv/bin/python paper/check_paper.py` agrees with that anchor here and exits **0**. The 0926Z paper lap installed `libreoffice-writer` in its own sandbox and took `check_paper.py`'s measuring branch, so the anchor is re-derived rather than hand-written — which is what the entry above asked for. **Your 25-page rule is verified again, with two pages to spare.** ⚠ This critic's sandbox has `soffice` but not the Writer import filter, so `measure_pages.py --why` reports 「no LibreOffice Writer」 here; that is the tool correctly distinguishing `libreoffice-core` from the filter, not a disagreement.
- **The word margin is now ZERO, for the first time.** `body_words` is **9000** in `paper/STATE.json` and `check_paper.py` re-derives **9000** here. `LIMIT` is 9000 and the test is `> LIMIT`, so 9,000 is inside by exactly one word of nothing. **The next paper lap that adds a single word is red**, and the two places that narrate this window both open by saying the margin is 1 (`paper/README.md:1514`; the header of `STATE.json`'s own `built_pages_note`) because the reviewer's `responder` repair landed after those sentences were written. The corrected pair is in the same paragraph at `paper/README.md:1560`. That bookkeeping half is **WFG-275**; this half is yours.

**What it changes for your decision, and it is the sharpest the entry has been.** The entry has always asked whether an 8,500/9,000-word proxy should stop a manuscript that the real rule (25 pages) says has **two pages** of room. Until today that was an argument. Today it is a bill: the window's strongest new evidence **runs against this paper's own number** — of the 24 origins it reports with no confirmed survival-aware ingress, the run's own corridor screening recorded **4** with a corridor surviving to or past the responder's ETA and **1** reachable with the full 12-minute margin — and the paper lap **measured the sentence carrying it at +32 words and reverted it**, and the bare register clause alone at +10, and reverted that too. Both were refused for want of margin against the proxy, on a document the real limit says is two pages short. **A limitation this project found against itself is being withheld by a proxy, not by a page count.** Nothing was compressed and no caveat was traded, which is the right behaviour under the rule as written; the rule is what is now costing something measurable.

⚠⚠ **The options are the ones already on this entry (A, B, C, D above) and critic #74 deliberately does NOT restate or renumber them.** Its first draft invented a competing A/B/C set and caught it while reading the outgoing email back — two option sets under one id would have made 「NH-037: C」 unmappable, which is the WFG-138 shape this file exists to avoid. **What the new measurement does is move the weight between the existing options, and it moves it onto B.** Option **B** is 「land WFG-116's open half so a clean clone *measures* and the proxy stops being load-bearing at all」, and as of today that is **demonstrated rather than proposed**: the 0926Z paper lap installed `libreoffice-writer` in its own ephemeral sandbox, `check_paper.py` took its measuring branch, and `built_pages` came back **23** with a re-derived digest. **The thing option B asks a workflow to do has now been done by hand, once, successfully.** Option **A**'s 9,461-word raise is still the honest interim if you want one; **D** (「a lap that runs out of words trims, and reports what it trimmed」) is the option the zero margin now prices, because the only thing left to trim is caveats.

**Severity: raised to HIGH by critic #74.** It was MEDIUM while the cost was hypothetical. The margin is 0, the sprint ends **2026-09-15**, and the first thing the ceiling has actually refused is a limitation.
...
⚠⚠ **CRITIC #74, 2026-09-12T1125Z — the queue this entry describes has produced its first WRONG INSTRUCTION IN THE JUDGE'S HANDS, and the loop then declined to merge the correction by reading this entry as blocking a routine it does not constrain.** Measured at `092c907`.

**The defect, on paper.** `docs/auto/JUDGE_QA.md` Q39 (`:1554`) tells the student: the three biggest clusters ship as pre-built `dispatch_a4.pdf`, print those and regenerate the other thirty. That instruction builds a stack in which **one sheet prints 「예산 내 차량 진입로가 화재로 차단됨(우회 포함)」 and thirty print 「어느 거점에서도 생존 인지 차량 진입 경로가 확인되지 않음」 for the same code condition.** Q39 is source **3 of the 7** in `manifest_20260911T2137Z.json` (kit `WFG_printables_20260911T2137Z.pdf`, **60** pages, re-hashed **7 of 7** here) and that kit is named by `release/kcf-finals-2026/MANIFEST.json` (re-hashed **19 of 19** here). So the instruction is not only in the tree, it is in the box the student carries to Gwangju.

**The correction exists and is paste-ready.** `docs/auto/JUDGE_QA_PENDING.md` **P-005**, written by the 0920Z dev lap, replaces exactly one Q39 paragraph with 「print nothing pre-built, regenerate all 33」 plus the sentence to say to a judge already holding a mixed stack. The file now holds **five** cards, P-001 to P-005.

**And here is the part that is about this entry rather than about the sheets.** The 0920Z lap wrote that it drafted P-005 「and **not merged**, because NH-049 is open and `JUDGE_QA.md` is one of the seven printable sources」. **This entry does not constrain a dev lap.** Its own text says 「**the critic** has exactly two legal moves and neither is a card」, and its recommended **option A** says the merge is done by 「**the next dev lap that rebuilds the kit**」 — that is WFG-205. A dev lap merging P-005 and paying `make printables` is option A being **executed**, not decided. The lap was being careful and the caution cost a window on the one surface CHARTER §14b names first.

**What that adds to your decision.** The entry's severity was set at MEDIUM on the ground that 「nothing is red and nothing is lost」. That is no longer true: what is queued now includes a **wrong instruction already printed**, and the queue has also started to deter the routine that was always allowed to clear it. **Severity: raised to HIGH.** Option **A** is unchanged and still recommended, and it now needs one added clause — that the routine prompts say in so many words which routines may merge the staging file — because the ambiguity, not the gate, is what cost this window.

⚠ **Critic #74 did not merge P-005 itself**, for the reason this entry exists: the merge forces `make printables` and a re-pointed `release/kcf-finals-2026/MANIFEST.json`, and the second path is outside `docs/auto/`. It is this lap's one `fix-before-next-row` item for the dev lap instead, with one correction to the draft measured here (of the three pre-built PDFs, **exactly one** carries the superseded sentence, not three).
