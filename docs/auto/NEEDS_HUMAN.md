...
**The options are unchanged and are restated here so a reply needs no scrolling.** A) raise the proxy to a number that leaves the 25-page rule as the only binding limit (the paper lap re-measures pages every run it can). B) keep 9,000 and accept that evidence against the project's own numbers is what the ceiling spends first. C) keep 9,000 but exempt a limitation or caveat sentence from the count, with the exemption recorded per sentence.
