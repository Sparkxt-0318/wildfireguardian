...
| R2 | The finals screen shows the evidence cards that exist today: operating point (WFG-019), reconciliation (WFG-018), detection floor (WFG-021), horizon grounding, refuge placement; rebuilt with `--verify` | Nothing in `web/` changed since the baseline. The detection-floor card is WFG-021 part (a); part (b), the tests, landed at `f5f8498`, but the row is stranded in `in-progress` with no lap holding it (critic F15 / WFG-047), so **this line is blocked on a status word, not on work**. Unblocking it is minutes | ☐ |
| R3 | `make all-checks` green on a clean clone (CI) and on the booth laptop recipe in `docs/auto/finals/BOOTH_SETUP.md` | **Red on a clean clone at `8d1decf`, checked this lap:** `gates.py --mode full` exit 1, `1 failed, 1152 passed, 62 skipped`; `tests/test_rescue_lineage_ssot.py` fails on `docs/auto/reports/2026-09-03T2053Z-dev.md:194` (critic F13). One prose annotation fixes it. `baseline-verify` WARN is expected off-laptop and is a soft step | ☐ |
