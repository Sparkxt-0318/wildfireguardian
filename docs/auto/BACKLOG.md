...
| one-sided claims pass | variant B requires a machine-detection noun AND a human-channel noun in one sentence. That is what took the rule from 37 noisy hits to 4; it also means 「we designed the interface report-first」 passes | not fixable inside variant B. Needs the value-driven instrument, WFG-071 |
