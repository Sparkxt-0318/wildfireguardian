...
| WFG-117 | **P0** | KCF | **The Q&A bank drills the student, at T0, to say three registry counts that the screen behind them contradicts.** `docs/auto/JUDGE_QA.md` Q30 is the question about why today's numbers should be believed, and its drafted answer says 「등록된 값 295개 중 261개가 ... 나머지 34개는 「확인됨, 재현 불가」」, then splits the 34 into 16 (the overwritten OSM graph) and 18 (submission-reconciliation values). Counted here from the file rather than read from a lap: `docs/NUMBERS.json` `numbers` holds **326** entries, **268** with `reproducible: true` and **58** with `reproducible: false`. So all three spoken figures are stale, and the 16 + 18 decomposition accounts for 34 of 58 rather than all of them. ⚠ The same screen the student is standing in front of prints the corrected pair in the 검증 레지스트리 card (`web/finals.html` payload `registry.n_entries` 326, `registry.n_reproducible` 268), so a judge who looks up while the student speaks sees two different answers to 「얼마나 재현됩니까」. This is the WFG-057 failure shape (the bank quoting a count of the tree that the tree has moved past), on a **T0** question, on the one subject this project's credibility rests on. Nothing gates it: `tests/test_judge_qa_bank.py` contains no check that reads `docs/NUMBERS.json` counts, so the bank and the registry can drift apart indefinitely, which is how they got 31 apart. A ⚠ 근거 없음 note was added to Q30 by critic #21 so the student does not rehearse the numbers before the row closes. **Done when:** Q30's answer states counts that a test derives from `docs/NUMBERS.json` at run time (or states no count and points at the card), the non-reproducible decomposition is either recounted against the 58 or withdrawn in writing, and a test in `tests/test_judge_qa_bank.py` fails when any registry count written in the bank disagrees with the registry — graded by editing one digit in the bank and one entry's `reproducible` flag and seeing it go red both ways. ⚠ **RE-SCOPED BY CRITIC #22, 2026-09-05T2330Z: this row got bigger in one lap, and the way it grew is the argument for the run-time-derived fix rather than a corrected literal.** Critic #21 wrote a ⚠ note giving the student the corrected pair (326 / 268) and telling them to point at the screen's 검증 레지스트리 card for the exact figure. One dev lap later that advice is wrong on both halves: WFG-114 (`c8a3eee`) registered **57** `pp_uiseong_*` keys, so `docs/NUMBERS.json` at `f118bfe` holds **383** entries / **325** reproducible / **58** not (counted here), while `web/finals.html` still prints **326 · 268** because the screen has not been rebuilt since. **Three different counts now answer one T0 question**, and the newest of them was written by the lap that was fixing the problem. The Q30 note has been rewritten to tell the student to quote no count and to point at no screen. **Added to done-when:** the row must also state, in the bank, that the count changes every lap that registers a key — a corrected literal is a defect with a shorter fuse, not a fix. The screen half of the same drift is **WFG-113** | todo | true | one lap | B/A 제출 자료 · R5 · R1 |
| WFG-115 | **P0** | KCF | **The judged screen prints a second commit id, and it is not on this branch.** The 검증 레지스트리 evidence card renders a provenance line 「built at commit 41498ef」 (`web/finals.html:1924`, `scripts/finals.template.html:1924`, from `scripts/build_finals.py:628` -> `docs/NUMBERS.json` `built_at_git_commit`). Checked here, not reasoned about: `git cat-file -t 41498ef` is `commit`, `git merge-base --is-ancestor 41498ef HEAD` **exits non-zero**, and `git branch -a --contains` puts it only on `origin/auto/lap-b1989d5-superseded` and `origin/ordering-boundary`. That is the exact WFG-067 failure shape — a stamp that exists but is not reachable — in the same panel WFG-067 was about, and `tests/test_finals_screen.py` never reads this field: both of its ancestry assertions (`:544`, `:649`) read `_payload()["git"]`, the other stamp. ⚠ It is also **stale by construction**: `built_at_git_commit` was written when the registry held 153 entries and the card beside it now prints 326, because `build_numbers.py` is deliberately not re-run (WFG-040). And it falsifies part of a judge-facing answer: `docs/auto/JUDGE_QA.md` Q35 (T1, drill round 3) says the panel's hash 「이 저장소에 실제로 있고 **현재 브랜치에서 닿는** 커밋입니다」, true of one stamp and false of this one, and its 「없는 것」 line disclaims only byte-identity. **Done when:** the card either drops the line, or prints a stamp that `merge-base --is-ancestor ... HEAD` accepts with what it means beside it; a test asserts ancestry for **every** commit-shaped field in the payload rather than the one named field, graded against a stamp on another branch and a stamp with no matching object; Q35's answer and 「없는 것」 line are corrected to what the gate then proves | todo | true | one lap | B/A 제출 자료 · R1 · R5 |
