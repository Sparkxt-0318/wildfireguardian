...
- **DONE 2026-09-05.** `docs/auto/DEMO_SCRIPT_5MIN.md` (DRAFT-labelled per CHARTER §9): six timed segments summing to exactly 300 s over `docs/FINALS_DEMO.md`'s four acts plus an opening and a limits close; one interruption sentence for each of the five judge lenses; a §3 mapping table of **31 spoken figures**, each with a registry key or a named artifact and each marked 화면 or 구두. `scripts/check_screen_assets.py web/finals.html` PASS.
- **The audit half found no screen change was warranted, and that is the finding.** Measured, not assumed: **40 of the 41 keys in `build_finals.py` `REGISTRY_KEYS` are present in the built `web/finals.html`**; the one absent (`mr_yeongdeok_walk_time_increase_pct`) has never existed in the registry and no card reads it, so nothing on screen is wrong — filed as **WFG-090**, P1. **WFG-067 is closed on disk**: the SYSTEM INTEGRITY panel stamps `d5e2562`, which resolves and is an ancestor of HEAD; the notes on `KCF_READINESS.md` R2 from critics #9/#10/#11 naming `a562045` were true when written and are stale now. A rebuild was run, produced a one-line diff (a fresher stamp) and was **reverted**, because regenerating a committed build product to buy nothing is the churn CHARTER §3.2 exists to prevent. So critic #14's cheapest test resolves as: the script exists, and the `web/` commit it predicted was **not** warranted.
- **What binds it:** `tests/test_demo_script_5min.py` (9 tests) reads the §3 table mechanically — every cited key must resolve in `docs/NUMBERS.json`, every 화면 row's key must be present in the built `web/finals.html`, the timings must be contiguous and total 300 s, the losing dispatch-ordering result must stay in the script, and the builder's `default_region` must still be the region act 3 quotes. Graded against six mutations written to break it (fake key, 구두→화면 relabel, timing drift, dropping the losing result, dropping a judge row, relabelling every row 화면): **6 of 6 caught.**
