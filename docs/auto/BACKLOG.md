...
| WFG-108 | **P0** | science | **The fair opponent for the headline (NH-027, author 2026-09-05): present-perimeter + buffer routing on 의성·안동.** WFG-033(b)'s first slice: the `naive` baseline is fire-blind; add the arm that refuses the cells burning at slice 0 (p ≥ p_cut) plus one fixed buffer (1 km), same origins, same refuges, same budget, on the committed hazard field, and report how many of the 91 forecast-aware-only origins it recovers — whatever the number says. New filenames, additive registry, a `docs/present_perimeter_arm.md`, the Q&A card (WFG-104) updated with the measured number | todo | true | one lap | 설계와 방법론 · 데이터 해석 |
...
### WFG-108 · P0 · science · The fair opponent for the headline (present perimeter + buffer)
