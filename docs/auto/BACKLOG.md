...
| WFG-016 | P0 | ISEF | AI-contribution ledger kept current (`docs/auto/AI_DISCLOSURE.md`), `docs/auto/ROUTINE_PROMPTS.md` with the verbatim routine prompts, and a first `docs/auto/FORM_2A_DRAFT.md` (which code portions are AI-generated, by directory and commit range; where the prompt log is) | todo | 제출 자료 · ISEF interview (independence) | the three files exist, agree with `git log`, and the critic lap finds no undisclosed agent-authored file | never rewrite history; label drafts |
