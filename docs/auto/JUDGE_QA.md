...
⚠ **근거 없음 (크리틱 #21, 2026-09-05, WFG-117). 위 초안의 세 숫자를 부스에서 말하지
마십시오.** 초안이 말하는 295 / 261 / 34는 옛 값이고, 34를 16 + 18로 나누는 뒷문장도
맞지 않습니다. 이 셋은 어떤 게이트도 읽지 않습니다(`tests/test_judge_qa_bank.py`에
레지스트리 수를 읽는 검사가 없습니다).

⚠⚠ **크리틱 #22, 2026-09-05T2330Z: 바로 위 경고문이 적어 둔 대체 숫자도 이미 낡았고,
그것이 WFG-117의 진짜 크기입니다.** 크리틱 #21은 이 자리에 「등록 326 · 재현 가능 268 ·
재현 불가 58」이라고 적고 「정확한 수는 화면의 검증 레지스트리 카드에 있으니 화면을
가리키라」고 했습니다. 그 사이 **개발 랩 한 번**(WFG-114, `c8a3eee`)이 `pp_uiseong_*` 키
**57개**를 등록했습니다. `f118bfe`에서 이 랩이 `docs/NUMBERS.json`을 직접 센 값은
**383 · 재현 가능 325 · 재현 불가 58**입니다. 그리고 `web/finals.html`의 검증 레지스트리
카드는 아직 **326 · 268**을 인쇄합니다 — 화면이 그 뒤로 다시 빌드되지 않았기 때문입니다.

**그래서 지금 이 한 질문에 서로 다른 수가 셋 있습니다: 초안 295/261, 화면 326/268,
레지스트리 383/325.** 화면을 가리키라는 조언은 이제 학생을 낡은 숫자로 데려갑니다.

**WFG-117이 닫힐 때까지 할 일: 어떤 수도 외워서 말하지 말고, 화면도 가리키지 마십시오.**
이렇게만 말하십시오 — 「등록된 값의 대부분이 커밋된 산출물에서 `make verify`로 다시
계산되고, 나머지는 '확인됨, 재현 불가'로 라벨이 붙어 있습니다. 정확한 개수는 랩마다
늘기 때문에 지금 이 자리에서 숫자를 외워 말씀드리지 않겠습니다. `docs/NUMBERS.json`을
열어서 함께 세어 보셔도 됩니다.」 마지막 문장은 물러서는 말이 아니라 이 프로젝트가
실제로 할 수 있는 유일한 자랑입니다.

⚠⚠⚠ **크리틱 #26, 2026-09-06T1100Z — 바로 위 두 개의 ⚠ 블록은 이제 낡았습니다.
지우지 않고 기록으로 남기지만(CHARTER §3.7), 그 안의 세 문장은 오늘 사실이 아닙니다.**
WFG-113 이 `1ec1d06` 에서 화면을 다시 지었습니다. 이 랩이 한 프로세스 안에서 직접 센 값:
`docs/NUMBERS.json` 은 **383 · 재현 가능 325 · 재현 불가 58**, `web/finals.html` 의 검증
레지스트리 카드도 **383 · 325**. **둘은 일치합니다.**

그러므로 다음 세 문장을 읽고 외우지 마십시오:

- 「화면은 아직 **326 · 268** 을 인쇄합니다」 — **아닙니다.** 383 · 325 를 인쇄합니다.
- 「화면을 가리키라는 조언은 학생을 낡은 숫자로 데려갑니다」 — **아닙니다.** 화면은
  오늘 레지스트리와 같은 수를 말합니다.
- 「서로 다른 수가 셋 있습니다」 — **아닙니다.** 둘입니다. 낡은 쪽은 이 카드 맨 위
  초안의 **295 / 261 / 34** 이고, 그것만 말하지 않으시면 됩니다.

**그래서 부스에서 할 일은 위 「WFG-117이 닫힐 때까지 할 일」 문단 그대로입니다** — 숫자를
외워 말하지 않고 함께 세어 보자고 하는 답. 그 답은 여전히 가장 안전하고, 랩마다 수가 늘기
때문에 앞으로도 그렇습니다. 달라진 것은 하나뿐입니다: **심사위원이 화면을 가리키신다면
이제 그 화면을 믿으셔도 됩니다.** 「화면도 가리키지 마십시오」는 오늘로 필요 없어졌습니다.
...
그리고 재현 불가 58개가 어떤 이유로 나뉘는지는 아직 세어 본 랩이 없습니다.
