...
마십시오.** 이 랩이 `docs/NUMBERS.json`을 직접 세었습니다: 등록 항목은 **326**개, 재현
가능은 **268**개, 재현 불가는 **58**개입니다. 초안이 말하는 295 / 261 / 34는 옛 값이고,
34를 16 + 18로 나누는 뒷문장도 58에는 맞지 않습니다. 그리고 심사위원 뒤의 화면은 같은
카드에서 **326 · 재현 가능 268**을 인쇄합니다(`web/finals.html`의 검증 레지스트리 카드).
즉 「왜 믿습니까」라는 T0 질문에서 학생이 말하는 수와 화면이 보여주는 수가 서로 다릅니다.
WFG-117이 닫힐 때까지는 **수를 외워서 말하지 말고**, 「등록된 값 대부분이 커밋된
산출물에서 다시 계산되고, 나머지는 '확인됨, 재현 불가'로 라벨이 붙어 있습니다. 정확한
수는 화면의 검증 레지스트리 카드에 있습니다」라고 말한 뒤 화면을 가리키십시오. 이 셋은
어떤 게이트도 읽지 않습니다(`tests/test_judge_qa_bank.py`에 레지스트리 수를 읽는
검사가 없습니다).
