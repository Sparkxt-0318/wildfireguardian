...
없는 것: 각인이 실재하는 커밋을 가리키는지 확인하는 게이트. `tests/test_finals_screen.py`
26개 중 어느 것도 그 줄을 읽지 않습니다. 고쳐지면 이 항목은 「예, 받아서 `make finals` 를
돌리면 같은 파일이 나옵니다」 로 바뀌어야 하고, 그 전까지는 바꾸지 마십시오.
