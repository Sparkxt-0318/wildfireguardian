...
신고보다 몇 시간 전 스텝이 들어 있고, 주간에는 태양 반사만으로 배경 대비가 +6…+8 K 라
3.09 K 는 지나치게 헐거운 문턱입니다. 그래서 이 숫자는 「문턱이 올라간 것이 원인」이라는
증거이지 「깨끗했다면 영덕을 탐지했다」는 뜻이 아닙니다. 그리고 설령 그 이상이 진짜 화재였다 해도
...
> ⚠ **근거 없음 — 이 질문은 지금 파일로 답할 수 없습니다.** 그리고 이 파일의 Q10과
> `docs/auto/finals/DETECTION_FLOOR_CARD.md`는 지금 답할 수 없는 것을 답하고 있습니다.
> 수정은 백로그 **WFG-053**이고, 그 전까지는 아래의 좁은 답만 쓰십시오.

답변(초안, 좁은 형태): "기록된 **발생일시** 기준입니다. 그 값이 신고 시각인지 발화 시각인지는
...
(`first hit (2025-03-25) lags the 2025-03-22 ignition by days`). 그래서 논문은 이것을
「기록된 발생일시 대비 지연」이라고만 쓰고 위성이 신고보다 빨랐는지 늦었는지는 주장하지
않습니다. 어느 쪽으로 읽어도 남는 결론은 크기 바닥입니다 — 2 km 화소는 대략 1 ha 아래의 불을
