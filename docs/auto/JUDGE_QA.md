...
**Q35 · T1 (크리틱 #8). "화면 아래 「commit a562045」 라고 적혀 있습니다. 그 커밋을 받아서
이 화면을 그대로 다시 만들 수 있습니까?"**

> 🛑 **근거 없음 - 오늘 이 질문에는 「아니오」 라고 답해야 합니다 (백로그 WFG-067).**
> `web/finals.html` 의 신뢰성 탭 SYSTEM INTEGRITY 패널이 인쇄하는 `a562045` 는 이 저장소에
> 존재하지 않는 객체입니다. 신선한 클론에서 `git cat-file -t a562045` 는
> `fatal: Not a valid object name` 을 돌려줍니다. 화면을 만든 랩이 빌드한 뒤 `git pull
> --rebase` 를 했고, 리베이스가 그 커밋을 버렸는데 빌드된 HTML 안의 각인은 그대로 남았기
> 때문입니다 (`scripts/build_finals.py` 의 `git_head()` 가 빌드 시각의 `HEAD` 를 읽습니다).

답변(초안, 고쳐지기 전까지): "그 줄은 지금 틀렸습니다. 빌드 뒤에 리베이스가 있었고 각인이
따라가지 못했습니다. 화면에 있는 **수치**는 전부 재현 가능합니다 - 그 아래 세 줄의
`verify-numbers` · `check-forbidden` · `check-region-literals` 는 빌드할 때 실제로 실행된
게이트의 종료 코드이고, `DATA` 세 줄의 sha256 은 커밋된 산출물의 것입니다. 재현 가능하지
않은 것은 **각인 한 줄**이며, 그것을 잡는 테스트가 없다는 것이 저희가 이미 적어 둔
백로그 항목입니다."

근거: `docs/auto/BACKLOG.md` WFG-067, `scripts/build_finals.py` `git_head()`,
`docs/auto/finals/screens_20260904T0630Z/4_reliability.png` (그 화면의 커밋된 스크린샷).
...
추가하고 백로그 행을 만듭니다. **2026-09-04 (크리틱 #7) 기준으로 두 개가 열려 있습니다:**

| 질문 | 왜 닿지 않는가 | 백로그 |
|---|---|---|
| **Q10d · T0** 사람 쪽을 앞세우는 이유를 묻는 질문 | 크기 바닥은 위성을 배제할 뿐 사람을 옹립하지 않고, 그 자리를 메우던 통계는 미등록 잠정치로 판정되어 금지되었습니다 | WFG-063 |
| **Q34 · T2** 「확산 속도는 시간당 얼마였습니까?」 | 저장소가 잰 값이 아니고, 보도된 값은 지식 노트에만 있으며 아직 등록되지 않았습니다 | WFG-065 |

두 질문 모두 **말하면 안 되는 문장**을 각 항목에 ❌ 로 적어 두었습니다. 근거가 없을 때
가장 비싼 실수는 침묵이 아니라 즉흥적으로 메우는 것입니다.

⚠ **알파벳이 붙은 질문(Q10a·Q10b·Q10c·Q10d·Q30a·Q30b)은 위 드릴 표와
`tests/test_judge_qa_bank.py` 의 계수·연속성 검사에 잡히지 않습니다** (WFG-057). Q10d 는
T0 이므로 1회차 드릴에 **손으로** 넣어 연습하십시오.
