...
이 표가 있는 이유는 단순하다: 세 자리 수치 하나가 어느 범위의 값인지 밝히지 않으면
심사위원이 한 번의 검색으로 반증할 수 있고, 실제로 이전 판본이 그랬다. 아래 A와 B는
**서로 다른 사건 범위**이며 절대 섞어 쓰지 않는다.

### A. 의성발 경북 산불 (의성→안동→청송→영양→영덕) — 이 프로젝트의 대상 사건

3월 22일 11:24 의성군 안평면 발화, 3월 28일 17:15 진화 완료.

| 항목 | 값 | 출처 | 기준일 | 비고 |
|---|---|---|---|---|
| 피해면적 | 45,157 ha | 중앙재난안전대책본부 | 2025-03-27 | **진화 완료(3-28) 이전** 집계 |
| 피해면적(교차) | 48,000 ha 이상 | World Weather Attribution 신속연구 | 2025-04-30 | 분석 대상 화재군 기준 |
| 사망 | 26명 | 중대본 최종 집계 | 2025-03 말 | 영덕 8·영양 6·안동 4·청송 3 등 |
| 주택 전소 | 150동 | 산림청 | 2025-03-26 | 시설 전소 합계 275동 |
| 시설 반소·부분소실 | 330동 | 산림청 | 2025-03-26 | |
...
- **116,000 ha는 어느 범위에도 해당하지 않는다.** 이전 README가 A 범위 문장에 이 값을
  썼으나, 전국 합계(104,788 ha)보다도 크다. 폐기.
- **"주택 4,000여 채"는 A가 아니라 B의 값**(3,848동)이다. A의 주택 전소는 150동이다.
- **사망자 27명도 어느 쪽도 아니다.** A는 26명, B는 32명.
- WWA 페이지는 "32 casualties, 26 of them in Uiseong-gun"으로 적는데, 여기서
  "Uiseong-gun"은 의성 발화 화재군 전체를 느슨하게 가리키는 표현으로 읽어야 하며
  행정구역 의성군만을 뜻하지 않는다.

**남은 확인 과제 (WFG-043):** A의 최종 피해면적을 산림청 1차 자료(보도자료·통계연보)에서
확정할 것. 현재 45,157 ha는 진화 완료 전 집계이고 WWA는 48,000 ha 이상을 제시하므로,
둘 사이의 최종 확정값이 존재할 가능성이 높다.
