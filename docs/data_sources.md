...
vs 전국 합계), 기간도 다르다(3월 6일간 vs 봄철 전체).

**대체 비율도 쓰지 않는다(정정, 2026-09-04).** 위 문단은 한동안 「같은 기준으로 맞추면
약 43 %」라는 대안을 함께 실었다. 그 문장 역시 **철회한다**. 45,157 ha는 B와 기준이
같은 값이 아니라 **합동조사가 뒤집은 이전 추정치**이며(함정 1이 이미 그렇게 적고 있다),
따라서 45,157 ÷ 104,788은 「기준을 맞춘 비율」이 아니라 **폐기된 분자를 현행 분모로
나눈 값**이다. 이 화재군의 합동조사 이후 「산불영향구역」 값은 이 저장소에 없으므로,
같은 기준끼리 맞춘 비율 자체가 존재하지 않는다. 자세한 근거는 함정 6.
...
1. **45,157 ha와 99,289 ha는 「틀린 값 / 맞는 값」이 아니라 서로 다른 양이다.**
   ...
   **두 값의 관계는 「어느 쪽이 항상 크다」가 아니다.** 두 출처를 각각 밝혀 둔다.
   ⓐ 정의 — 산불영향구역은 「산불진화 전략 수립을 목적으로 헬기영상, 드론영상, 지상관측
   등에서 파악한 **화선 경계**를 표시한 구역」이고, 피해면적은 「피해복구를 목적으로 실제
   **현장조사**를 통해 산불피해가 발생한 산림 면적을 따지는 것」이다
   (문화일보 2025-04-18, 김창희, [기사](https://www.munhwa.com/article/11499954),
   2026-09-04 열람).
   ⓑ 비교 가능성 — 산림청은 「산불영향구역과 피해면적은 개념이 달라서 **단순 비교할 수
   없고**」라고 밝혔고, 산림청 관계자는 「실제 피해면적은 **줄어들 수도 있고 늘어날 수도
   있다**」고 말했다(경향신문 2025-04-17, 김현수·이종섭,
   [기사](https://www.khan.co.kr/article/202504171020011), 2026-09-04 열람 — 이 문서가
   99,289 ha의 출처로 이미 인용하고 있는 바로 그 기사다).
   이 절이 45,157 ha를 「다른 기준의 값」이 아니라 **뒤집힌 추정치**로 다루는 이유는
   어느 쪽이 크냐가 아니라 목적과 산정 방법이 다르기 때문이다.
   README 서두는 99,289 ha를 쓰고 45,157 ha는 **「산불영향구역」이라는 기준과 시점을
   함께** 밝혀서만 인용한다.
...
6. **A와 B를 나누지 말 것 — 어떤 분자를 올려도 비율이 성립하지 않는다.** critic #4는
   「이 화재군이 전국의 약 95 %」를 README에 쓰라고 했고, 2026-09-04T0037Z 수동 랩이
   실제로 그렇게 썼다. 쓰지 않는다: A는 **6일간·경북 중심·산림피해 면적**이고 B는
   **봄철 전체(1/24~5/15)·전국·산림청 집계**라, 기간도 주체도 다르다.
   **정정(2026-09-04, critic #5 F21).** 이 항목은 한동안 「B와 같은 기준인
   산불영향구역(45,157 ha)으로 잡으면 약 43 %」라는 대안 비율을 제시했다. 그 대안은
   **틀렸고 철회한다.** 세 가지가 그것을 부정한다.
   ⓐ 함정 1이 이미 적고 있듯 45,157 ha는 B와 나란히 놓을 수 있는 다른 기준의 값이 아니라
   **합동조사가 두 배 넘게 뒤집은 이전 추정치**다. 한 문서가 함정 1에서는 뒤집힌 추정치라
   부르고 함정 6에서는 나눠도 되는 기준이라 부르고 있었던 것이 이 오류의 실체다.
   ⓑ **산림청 자신이 두 개념은 단순 비교할 수 없다고 밝혔다**(함정 1 ⓑ의 경향신문
   2025-04-17 인용; 정의는 함정 1 ⓐ의 문화일보 2025-04-18). 목적(진화 전략 vs 피해복구)도
   산정 방법(화선 경계 관측 vs 현장조사)도 다르다. 「같은 기준으로 맞춘다」는 조작 자체가
   성립하지 않는다.
   ⓒ 분모 B에서 45,157을 빼고 남는 양은 그 시즌의 **나머지 화재 전부**가 나눠 가져야 하는
   몫인데, 그러면 그 나머지가 전국 합계의 절반을 넘게 된다. 그 시즌 최대 화재가 바로 이
   화재군이다. 요컨대 45,157 ÷ B는 기준을 맞춘 비율이 아니라 **폐기된 분자 ÷ 현행 분모**다.
   이 화재군의 합동조사 이후 산불영향구역 값을 이 저장소는 갖고 있지 않으므로, 같은
   기준끼리 맞춘 비율은 아예 만들 수 없다.
   레지스트리의 `fire2025_chain_share_of_nationwide_pct`(94.8)는 산술 기록일 뿐이며
   판정용 산문에는 어느 비율도 쓰지 않는다.
