...
1. **45,157 ha와 99,289 ha는 「틀린 값 / 맞는 값」이 아니라 서로 다른 양이다.**
   ...
   **두 값의 관계는 「어느 쪽이 항상 크다」가 아니다.** 산림청 자신이 두 개념을 이렇게
   갈라놓았다(문화일보 2025-04-18, 김창희, [기사](https://www.munhwa.com/article/11499954),
   2026-09-04 열람): 산불영향구역은 「산불진화 전략 수립을 목적으로 헬기영상, 드론영상,
   지상관측 등에서 파악한 **화선 경계**를 표시한 구역」이고, 피해면적은 「피해복구를
   목적으로 실제 **현장조사**를 통해 산불피해가 발생한 산림 면적을 따지는 것」이며,
   따라서 **단순 비교할 수 없다**. 실제 피해면적은 영향구역보다 작을 수도, 클 수도 있다.
   이 절이 45,157 ha를 「다른 기준의 값」이 아니라 **뒤집힌 추정치**로 다루는 이유는
   방향이 아니라 목적과 산정 방법이 다르기 때문이다.
...
6. **A와 B를 나누지 말 것 — 어떤 분자를 올려도 비율이 성립하지 않는다.** critic #4는
   ...
   **합동조사가 두 배 넘게 뒤집은 이전 추정치**다. 한 문서가 함정 1에서는 뒤집힌 추정치라
   ...
   ⓑ **산림청 자신이 두 개념은 단순 비교할 수 없다고 밝혔다**(함정 1의 문화일보
   2025-04-18 인용). 목적(진화 전략 vs 피해복구)도 산정 방법(화선 경계 관측 vs 현장조사)도
   다르다. 「같은 기준으로 맞춘다」는 조작 자체가 성립하지 않는다.
