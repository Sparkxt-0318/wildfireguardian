...
**발견.** `README.md:731` 이 이전(superseded) 452계열의 `6 → 34` 를, 나머지가 모두 439계열인
...
계보 게이트** 입니다: 이전(superseded) 452계열 값이 옆에 계보 표시 없이
등장하면 실패합니다. 이 테스트는 켜자마자 위 1·2번을 스스로 찾아냈고,
독립 검토 뒤 조여서 두 곳(`BACKLOG_PROPOSAL`, `results_rescue_draft.md`)을 더
찾아냈습니다.

⚠ **이 게이트의 한계를 정확히 적습니다.** 면제는 파일 이름을 테스트 안에
**명시적으로 적은 경우에만** 적용됩니다(현재 2개, 각각 이유와 함께).
초판은 「첫 20줄에 SUPERSEDED 가 있으면 파일 전체 면제」였는데, 그러면
`docs/auto/JUDGE_QA.md` 와 `docs/HANDOFF_ROUND3.md` 를 포함해 **15개 파일이
우연한 단어 하나로 면제**됐습니다. 라벨 목록에서도 맨 `이전`·`정본` 을
뺐습니다 — 합성 계보 값을 **정본이라고 잘못 적은** 문장이 `정본` 이라는
단어 덕분에 통과했기 때문입니다. 둘 다 독립 검토가 찾았습니다.
...
- **pooled 0.905** (행 가중, 151,904행 1회씩) → 「증거 전체에 대한 **판별력**」.
  폴드 크기 왜곡이 없습니다. 대신 큰 폴드가 지배합니다 (행 비중: 의성·안동
  54.47 %, 울진·삼척 27.42 % — `docs/fold_sizes.md`).
...
| 파일 | 무엇 |
|---|---|
| `README.md` | §1 계보 혼입 정정 + 이전(superseded) 452계열 주석 |
