...
> 않는다. **킬로미터 거리는 여기에 쓰지 않는다.** 담기 주장에는 거리가 필요 없고, 유통되던
> 「약 45 km」(critic #11)는 이 파일들 위의 어떤 계산으로도 재현되지 않는다(실측: 가장 가까운
> 점이 상자에서 30.5 km, 가장 먼 점이 65.6 km). 등록되지 않은 수는 적지 않는다(CHARTER §3.3).
>
