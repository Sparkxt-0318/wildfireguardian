...
## 3. 결과 (2026-09-07T0059Z 빌드 — **인쇄할 것은 이 스탬프입니다**)

- **33 페이지** — 체크리스트 5, 대본 6, 질의응답 17, 대조표 3, 탐지 카드 2. 합이 33 입니다.
- 폰트는 PDF 안에 임베드(Type 42)됩니다.
- 이 숫자들은 `manifest_20260907T0059Z.json` 의 `pages` 와 `pages_per_source` 에서 다시 읽은
...
- **`docs/auto/KCF_READINESS.md` R7 이 세는 다섯 가지 중 둘이 여기에 없습니다.** 관련연구 ·
  SFTD059T 차별화 패널(WFG-026)은 문서 자체가 아직 없습니다. `outputs/dispatch*` 의 29 장 마을
  A4 시트와 마을방송 대본은 **이미 커밋된 PDF** 라서 그대로 인쇄하면 되고, 여기에 다시 넣지
  않았습니다(사본 두 벌 금지, CHARTER §3.2). 나머지 셋 — 부스 체크리스트, 그리고 하나의 문서인
  「A4 근거 시트」와 「대조표」 — 은 들어 있습니다.
