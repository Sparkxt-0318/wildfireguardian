...
PDF 를 쓰고, **fontTools + brotli** (둘 다 이미 선언된 의존성) 가 `web/assets/fonts/` 의 한글
웹폰트를 풀어 줍니다. 그래서 인쇄물은 화면과 **같은 서체**, 그림과 **같은 스택**으로,
**새 의존성 없이** 나옵니다. 인터넷도, 키도, 유료 서비스도 쓰지 않습니다.
...
있습니다. 이 문서들이 쓰는 한글 음절은 전부 들어 있지만, 기호 17 개가 **없습니다**:
`§ — – ← → ≥ ⚠ 「 」 〔 〕 Ⅱ Ⅲ Ⅴ ② ✅ ❌ ⭕ 🛑`.
