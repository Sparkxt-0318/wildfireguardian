...
⚠ **4 번은 2026-09-07T0032Z 빌드에서 처음 들어갔습니다 (WFG-130).** 그 전까지 이 표는 네 줄이었고,
...
## 3. 결과 (2026-09-07T0032Z 빌드)

- **33 페이지** — 체크리스트 6, 대본 7, 질의응답 18, 대조표 4, 탐지 카드 3.
- 파일 약 439 KB, 폰트는 PDF 안에 임베드(Type 42)됩니다.
- 이 숫자들은 `manifest_20260907T0032Z.json` 의 `pages` 와 `pages_per_source` 에서 다시 읽은
  것이지 옮겨 적은 것이 아닙니다. 앞 빌드(2026-09-06T0620Z, 29 페이지)는 지우지 않고 옆에
  남겨 둡니다 — CHARTER §3.2. 질의응답이 17 에서 18 로 는 것은 그 사이 카드가 늘었기 때문입니다.
