...
- **새로운 숫자도, 새로운 주장도, 새로운 출처도 없습니다.** 인쇄물은 커밋된 네 문서의 인쇄
  ...
  낡았는지 기계로 확인할 수 있습니다.
  ⚠ **「확인할 수 있습니다」는 「확인하고 있습니다」가 아닙니다 (개발 랩 2026-09-06T1230Z).**
  그 sha256 을 지금 트리와 대조하는 테스트는 없습니다. `tests/test_printables.py` 는 manifest 가
  **자기 안에서** 앞뒤가 맞는지만 보고, manifest 가 서술하는 파일들이 그 뒤로 바뀌었는지는 보지
  않습니다. **살아 있는 사례가 지금 하나 있습니다:** 이 랩이 WFG-117 로 `docs/auto/JUDGE_QA.md`
  를 고쳤고, `manifest_20260906T0620Z.json` 이 적어 둔 `2c845121…` 와 지금 파일의 `6c09095d…` 는
  다릅니다. 그런데도 인쇄물 테스트 전부가 초록입니다. 즉 **커밋된 부스 인쇄본은 지금 질의응답
  카드보다 낡았고**, 그것을 말해 주는 게이트가 없습니다. 새 스탬프로 다시 빌드하는 일과 그
  게이트를 만드는 일은 **WFG-130** 에 적어 두었습니다. 그때까지: 인쇄하기 전에
  `sha256sum` 으로 네 원본을 manifest 와 직접 대조하십시오.
