...
CANDIDATES = {
    ...
    "mati_gr_2018": {
        ...
        "why": "Coastal settlement; residents trapped between fire and sea.",
        "sources": ["not verified in this session — see report"],
    },
    "pedrogao_pt_2017": {
        "bbox": (39.900, -8.220, 39.980, -8.100),
        "event": "Pedrógão Grande, 17 Jun 2017",
        "why": "Deaths concentrated on a road during escape.",
        "sources": ["not verified in this session — see report"],
    },
    "yeongdeok_kr_2025": {
...
"""
    ...
    for key, body in (("buildings", q), ("highways", q2), ("refuge_pois", q3)):
        ...
        try:
            ...
            out[key] = None
            out[f"{key}_error"] = f"{type(ex).__name__}: {str(ex)[:120]}"
...
def main() -> int:
    ...
    for name, spec in CANDIDATES.items():
        ...
        b = c.get("buildings")
        c["passes_gate"] = bool(b is not None and b >= MIN_BUILDINGS)
        results[name] = {**spec, **c}
