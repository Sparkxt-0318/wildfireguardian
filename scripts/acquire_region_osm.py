...
* **3 retries, 30 / 60 / 120 s backoff.** After that the region is abandoned and
...
def with_retries(what: str, fn):
    """Call ``fn`` with 3 retries and exponential backoff. Raises on final failure."""
    last = None
...
def acquire(region: str, bbox, dest: Path) -> dict:
    ...
    for name, tags in POI_LAYERS.items():
        ...
        prov["layers"][f"{name}.geojson"] = {
            ...
            "n_features": int(len(gdf)), "bytes": out.stat().st_size,
        }
