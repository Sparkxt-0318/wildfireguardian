#!/usr/bin/env python
"""F2 — the KMA-forecast field (Part B of docs/auto/briefs/HINDCAST_CORRECTION.md).

The same leave-one-complex-out model as F1, on the same canonical 영덕 canvas, but with
...
ERA5: reanalysis is the very thing that made the committed field a hindcast, so an F2 that
...
wearing the word 「forecast」 in its filename.
    ...
    python scripts/run_forecast_track_f2_kma.py
"""
...
def refuse(why: str, detail: str = "") -> int:
    print("F2 (KMA forecast) DID NOT RUN.", file=sys.stderr)
    print(f"  reason: {why}", file=sys.stderr)
    if detail:
        print(f"  {detail}", file=sys.stderr)
    print(file=sys.stderr)
    print(f"  F2 needs the archived 동네예보 issued BEFORE T0, under {KMA_DIR.relative_to(REPO)}/", file=sys.stderr)
    ...
    print(f"    ({DOC} §1); an F2 that did that would be a hindcast wearing the word", file=sys.stderr)
...
def main() -> int:
    ...
    # ------------------------------------------------------------------ #
    print(f"[F2] manifest found: {MANIFEST.relative_to(REPO)}")
    print(f"[F2]   product={man.get('product')!r} issued_utc={man.get('issued_utc')!r} event={man.get('event')!r}")
    print(f"[F2]   files: {len(man.get('files', []))}")
    print()
    print("[F2] STOPPING: the archive is here, but the 동네예보 -> weather-feature mapping is", file=sys.stderr)
