...
digit, states a cost, and passes every check in this file. The generalisation is
the one ``docs/withdrawn_claims.md`` §4 already records for the withdrawn-claim
registry: these are copy-paste ratchets over spellings, not claim detectors, and
a rewording escapes. Two smaller ones, said for the same reason: the credential
check reads the *names* out of ``docs/REPRODUCE.md`` and would not notice a third
key becoming necessary without that table changing, and nothing here can tell
whether the four-item list is *complete* -- only that each item it states is
true of the tree.
...
def test_the_two_credentials_are_the_two_the_reproduce_page_requires() -> None:
    """The card promises *two free keys*; REPRODUCE §2 is the table that decides.

    Read from that table rather than retyped, so a third required credential
    turns the card red instead of leaving it quietly wrong.
    """
    ...
    card = _card()
    families = {"FIRMS", "CDS"}
    named = {f for f in families if f in card}
    assert named == families, (
        f"{QID} names {sorted(named)} of the credential families the REPRODUCE "
        f"table requires ({sorted(rows)}). The card's 「무료 키 두 개」 must name both."
    )
